...
- Slowlog 第一批 partial：`SLOWLOG HELP`、`SLOWLOG LEN`、`SLOWLOG GET`、`SLOWLOG RESET` 当前可用；slowlog 当前是 redis-uya 进程内固定容量 ring，记录执行命令与 runtime-measured `duration_us`，但耗时精度受当前毫秒级时间源限制，且不含 Redis 原生阈值配置和客户端端点/名称真值
- Latency 第一批 partial：`LATENCY HELP`、`LATENCY LATEST`、`LATENCY HISTORY`、`LATENCY RESET`、`LATENCY DOCTOR`、`LATENCY HISTOGRAM`、`LATENCY GRAPH` 当前可用；当前采样 `command` 事件的运行时耗时并保留进程内历史，`HISTOGRAM [command ...]` 返回 top-level 命令名的累计微秒桶并可由 `CONFIG RESETSTAT` 清理，耗时精度受毫秒级时间源限制，尚未实现 Redis 原生 `latency-monitor-threshold` 事件门限或子命令名粒度直方图
