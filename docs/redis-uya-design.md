...
> 版本: v0.9.0-planning
> 日期: 2026-04-30
> 当前开发工具链: Uya `1.0` 分支 `337a0edd` 同步副本（内置启动器与编译器均返回 `v0.9.9`）
...
- 长期目标：在核心场景上超过 Redis。
- `v1.0.0` 目标：先把单机核心做真、做稳、做快，不用“长期超过 Redis”的口号替代当前版本的真实完成度。
...
- 当前设计以工程内置 `./uya` 工具链为准；其能力边界以上游 `../uya` 的 `1.0` 分支同步结果和 redis-uya 实测为准。
- 状态记录：
  - 当前参考的 `../uya` HEAD 为 `337a0edd`
  - `./uya/bin/uya` 是静态启动器，实际构建由动态链接的 `./uya/bin/cmd/build` 执行
  - 两个产物均打印 `v0.9.9`，同步报告记录其尺寸、哈希、上游测试状态和 redis-uya 验证证据
