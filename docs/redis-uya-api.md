...
CLIENT LIST ID id ...
CLIENT KILL ip:port
CLIENT KILL [ID id] [ADDR ip:port] [LADDR ip:port] [USER username] [TYPE normal|master|replica|pubsub] [SKIPME yes|no]
...
- `CLIENT KILL` 当前支持 legacy `ip:port` 单目标地址形式，以及现代 `ID/ADDR/LADDR/USER/TYPE/SKIPME` 组合过滤；`LADDR` 使用 accept 时记录的服务端本地地址，`USER` 使用连接认证后同步到注册表的 ACL 用户名，`TYPE` 复用实时 Pub/Sub 分类，三者均支持多目标关闭；当前仍不支持 `MAXAGE` filter
