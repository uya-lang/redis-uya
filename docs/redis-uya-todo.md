...
- [x] 复制角色与状态机
- [x] `PSYNC` / backlog
- [x] `REPLCONF` 握手 partial：常见选项返回 `OK`，ACK/GETACK 具备连接级 offset 记录与 wire 回复语义
- [x] 全量同步
- [x] 增量同步
- [x] 复制心跳
- [x] 集成测试：主从一致性
...
- [x] AOF 持久化位置前置能力：writer 区分 appended/flushed/synced offset，显式 sync 会先 flush 再执行 `fsync(2)`，BGREWRITEAOF 切换时保留逻辑写入位置并重置 synced；自动 `no/always/everysec` 策略与 `WAITAOF` 接线继续后置
...
- [ ] 复制/持久化深化：RDB 二进制兼容推进、AOF rewrite 压测、复制断线重连、`SYNC` legacy full-sync bulk snapshot partial 已补；`REPLCONF ACK` 已补整数校验、无普通回复、仅已建立 `PSYNC` 的复制连接记录单调 ACK offset/最近 ACK 时间，`GETACK *` 已补 replica 按当前上游 offset 写回 `REPLCONF ACK` 的 wire 语义；`WAIT` 已补调用连接最后传播写入 offset、在线复制连接 ACK 聚合、按需 GETACK、阻塞唤醒与 timeout。后续继续补周期性 GETACK、`WAITAOF` 真实副本 AOF ACK 等待、`MIGRATE` 当前已按单机安全 profile 收口为 `standalone-error`，`FAILOVER` 当前已收口为无副本/未支持 controlled failover 错误面，后续再补真实 failover 状态机、只读副本限制
