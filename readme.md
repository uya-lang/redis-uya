...
- 控制面兼容子集：`CLIENT ID/GETNAME/GETREDIR/SETNAME/NO-TOUCH/INFO/LIST/SETINFO/HELP/KILL/PAUSE/UNPAUSE/TRACKING/TRACKINGINFO`、`HELLO SETNAME`、`RESET`、`CONFIG GET/HELP/RESETSTAT/SET/REWRITE`
...
- Client no-touch partial：`CLIENT NO-TOUCH ON|OFF` 当前可用，用于保存连接级兼容标志、进入 `CLIENT HELP` 与 `COMMAND*` 可见面；尚未接入对象访问路径的 LRU/LFU touch 抑制
