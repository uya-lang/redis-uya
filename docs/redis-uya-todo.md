...
- [ ] ACL 第一批：`ACL CAT/DELUSER/DRYRUN/GENPASS/GETUSER/HELP/LIST/LOAD/LOG/SAVE/SETUSER/USERS/WHOAMI` 默认用户控制面 partial 已补，默认用户命令级 `+cmd/-cmd` deny list、分类级 `+@category/-@category` deny list、`resetcommands` 默认命令规则恢复、`resetkeys/resetchannels/clearselectors/resetselectors` 固定默认视图兼容 no-op、`requirepass` 只读哈希标记回显、真实连接 `NOPERM` 最小权限拒绝路径和默认用户命令权限拒绝 `ACL LOG [count]/RESET` 基础审计字段与真实 client id 已补，后续继续补用户、密码写入、完整 Redis 分类授权模型、key pattern 权限、selector 权限、真实客户端地址审计字段和 ACL 文件加载保存
