...
- `FUNCTION` 与 `FCALL/FCALL_RO` 的最小 library 平面由 `connection.uya` 接管：固定容量进程内表保存 library 名、函数名、原始代码和规范化单条调用体，`LOAD/LIST/STATS/DELETE/FLUSH` 反映真实状态，`FCALL*` 复用脚本内部命令的 ACL、只读校验、AOF 及 replication backlog 实际数据效果传播；`HELP/DUMP/RESTORE/KILL` 仍由 `command/executor.uya` 提供兼容错误面。每库仅一个 `redis.register_function(... return redis.call(...) end)` 子集，library 元数据尚不进入 RDB/AOF/复制
