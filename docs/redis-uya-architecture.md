...
- `ACL CAT/DELUSER/GENPASS/HELP/LOAD/SAVE/USERS/WHOAMI` 仍由 `command/executor.uya` 执行；`ACL SETUSER/DRYRUN/LIST/GETUSER/LOG` 在真实连接路径由 `connection.uya` 接管默认用户的进程内命令 deny list 和拒绝日志，以便在命令执行前统一返回 `NOPERM` 并记录 `ACL LOG`。当前支持默认用户命令级 `+cmd/-cmd` 允许/拒绝、`ACL LIST` / `ACL GETUSER default` 回显 deny list、事务队列前置拒绝、脚本内部命令拒绝和默认用户命令权限拒绝 ring 日志；ACL 用户存储、密码管理、分类权限、key pattern 权限、完整 Redis ACL 审计字段和 ACL 文件加载保存后续再补
