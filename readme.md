...
- ACL 第一批 partial：`ACL HELP`、`ACL LIST`、`ACL USERS`、`ACL WHOAMI` 当前可用，用于暴露 ACL 控制面帮助、默认用户 config 格式、默认用户列表、当前默认用户和 `COMMAND*` 可见面；`ACL LIST` 当前固定返回 `user default on nopass ~* &* +@all`，暂不反映 `requirepass`；暂不支持 ACL 用户存储、命令权限、key pattern 权限、ACL 日志或 ACL 文件加载保存，安全基线仍由 `requirepass` / `AUTH` 提供
