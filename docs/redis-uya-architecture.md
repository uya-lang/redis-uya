...
- `ACL CAT/GENPASS/HELP/WHOAMI` 的无状态兼容面仍可由 `command/executor.uya` 执行；真实连接上的 `ACL SETUSER/DRYRUN/LIST/GETUSER/USERS/DELUSER/LOG/LOAD/SAVE` 由 `connection.uya` 接管默认用户命令/分类 deny list、key/channel pattern、named user 元数据、口令、认证和拒绝日志，以便在执行前统一返回 `NOPERM`。口令状态采用单摘要模型：复用 Uya 1.0 标准库 SHA-256，`>password` 立即摘要化，`#hash` 写入 32 字节摘要，认证对候选口令摘要后做全字节累计比较，`nopass/resetpass/<password/!hash` 会擦除摘要字节；兼容 `requirepass` 配置仍保留独立明文来源，后续 CONFIG 设置会清除 ACL 摘要。`aclfile` 进入 `ServerConfig`、运行时 CONFIG 与 rewrite；`ACL SAVE` 只输出小写 64 位 `#hash`，再以 mode `0600` 排他重建 `.tmp`、完整写入并关闭后原子 rename；`ACL LOAD` 兼容 `#hash` 与旧 `>plaintext` token，读取不超过 64 KiB 的文件，先快照全部 ACL 表和候选配置，解析成功才提交，任何文件/语法/容量错误都恢复快照。加载要求唯一 `default` 和唯一 named user 声明，启动参数指定 ACL 文件时由 `server_set_aclfile_and_load()` 在事件循环前执行，失败则拒绝启动。默认用户与 named user 分别维护活跃命令/分类拒绝规则计数；当前支持 `+cmd/-cmd`、`+@category/-@category`、`resetcommands`、`allkeys/resetkeys/~pattern`、`allchannels/resetchannels/&pattern`、named user 生命周期、单个 `>password/<password/#hash/!hash/resetpass/nopass`、`AUTH` / `HELLO AUTH`、事务/脚本权限检查和 ACL LOG 基础审计。多个并存口令、selector 权限和 Redis 动态 key spec / movablekeys 完整解析后续再补
