...
- 当前 `HEAD` 的完整单测和完整集成已通过；release 矩阵见 `benchmarks/v0.9.3-release-performance.md`。动态 RESP batch、空闲 MONITOR、两类空闲扫描、无效配置占位和单线程 async allocator 优化把最新正式 `SET 1KiB` 从 `2811` 提升到 `15661` req/s，本轮为同机 Redis 的 `1.17x` 且 p99 更低；两个 GET 已收敛到 Redis 的 `0.98x/0.99x`。
...
- 当前 `v0.9.3` 功能回归为绿态；最新正式 release 比例为 `PING 0.89x`、`SET 16B 1.09x`、`GET 16B 0.98x`、`SET 1KiB 1.17x`、`GET 1KiB 0.99x`。完整超越目标尚未达成，后续优先收敛重复时钟读取、运行时元数据装配、backlog 物理存储和内存占用。
