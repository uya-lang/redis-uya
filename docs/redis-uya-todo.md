...
- [ ] 复制/持久化深化：RDB 二进制兼容推进、AOF rewrite 压测、复制断线重连、`SYNC` legacy full-sync bulk snapshot partial 已补；`REPLCONF ACK` 已补整数校验、无普通回复、连接级单调 ACK offset/最近 ACK 时间记录，`GETACK *` 已补 replica 按当前上游 offset 写回 `REPLCONF ACK` 的 wire 语义；后续继续补 master 主动 GETACK、`WAIT` / `WAITAOF` 真实等待语义、`MIGRATE` 当前已按单机安全 profile 收口为 `standalone-error`，`FAILOVER` 当前已收口为无副本/未支持 controlled failover 错误面，后续再补真实 failover 状态机、只读副本限制
