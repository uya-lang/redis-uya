...
- [ ] ACL 第一批：控制面、默认/named user、基础 `-@all` whitelist 与有序 command/category allow/deny、key/channel pattern、多 SHA-256 口令、审计日志和 aclfile 原子 SAVE/全状态回滚 LOAD 已补；每用户最多 8 个 selector 已补，同一 selector 内完整匹配、多个 selector 与基础规则 OR 授权，普通命令、事务、脚本与 DRYRUN 共用统一判定，GETUSER/LIST/SAVE/LOAD、损坏回滚和重启加载均有覆盖。后续继续补动态 key spec / movablekeys 完整 key 解析
