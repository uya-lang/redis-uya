...
- 2026-07-27 当前 `HEAD`：`make test`、完整 33 项集成、redis-cli、DoD 与 release benchmark 全部通过；字典 hash 命中后的 key 比较已在查找/删除循环直接调用 `bytes_eq`，删除 release 中只负责 SDS slice 转换并尾跳的 28 字节 helper，hash、碰撞链和渐进 rehash 语义保持不变。两组相反顺序、共 8 次固定 CPU `GET 1KiB` 父提交 A/B 合并吞吐仅提升 0.67%，作为噪声背景；`perf stat` 的 cycles/instructions/branches 分别下降 2.48%/0.09%/0.09%。不可变与 current 50K 的五项绝对吞吐、p99 和总体 guard 均通过；current Redis 比值为 `1.02x/1.25x/0.96x/1.20x/0.99x`，两个 GET 场景仍未稳定达到严格 1.10x 全场景超越要求。
