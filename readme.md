...
- Geo 第一批 partial：`GEOADD`、`GEODIST`、`GEOPOS`、`GEOSEARCH` 当前可用，但内部暂以 exact zset-backed packed coordinate score 实现，`GEOPOS` 和 `WITHCOORD` 返回当前 packed score 解码后的 `1e-6` 量化坐标，`WITHHASH` 返回当前 packed score，而不是 Redis 原生 geohash 整数
