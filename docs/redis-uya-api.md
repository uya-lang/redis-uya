...
- `ACL GETUSER default` 返回当前默认用户详情，并在 `commands`、`keys` 与 `channels` 字段中反映当前命令/分类 deny list、key pattern 和 channel pattern 规则；启用 `requirepass` 或 ACL 哈希口令时 `flags` 不再包含 `nopass`，`passwords` 返回 `#` 加 64 位小写十六进制 SHA-256 而不暴露明文；已创建 named user 返回 `flags/passwords/commands/keys/channels/selectors` 元数据视图，`>password` 写入的口令会立即转为 SHA-256，`#hash` 可直接写入摘要；未知用户名返回 null
...
- `ACL SETUSER default [attribute ...]` 当前支持 `on`、`nopass`、`resetpass`、`>password`、`<password`、`#hash`、`!hash`、`&*`、`+@all`、`clearselectors`、`resetselectors` 兼容面，也支持命令级 `-cmd` / `+cmd`、分类级 `-@category` / `+@category`、`resetcommands`、`allkeys`、`resetkeys`、`~pattern`、`allchannels`、`resetchannels` 和 `&pattern`；`>password` 会立即转为 SHA-256，`#hash` 只接受 64 位小写十六进制，`<password` / `!hash` 删除不存在的口令时返回 Redis 兼容错误。default 的 ACL 哈希口令不通过 `CONFIG GET requirepass` 反向暴露；后续 `CONFIG SET requirepass` 会切换回兼容明文配置来源并清除旧摘要。例如 `ACL SETUSER default -get` 或 `ACL SETUSER default -@string` 会让 default 用户连接的后续 `GET` 与 `ACL DRYRUN default GET ...` 返回命令 `NOPERM`
- `ACL SETUSER <named-user> [attribute ...]` 当前支持进程内 named user 元数据创建与更新，覆盖 `on`、`off`、`nopass`、`resetpass`、`>password`、`<password`、`#hash`、`!hash`、`~*`、`&*`、`+@all`、`allkeys`、`resetkeys`、`~pattern`、`allchannels`、`resetchannels`、`&pattern`、`clearselectors`、`resetselectors`、`-cmd/+cmd` 和 `-@category/+@category`；口令在内存中只保存单个 SHA-256 摘要，`AUTH username password` / `HELLO ... AUTH username password` 对候选口令计算摘要后比较，`nopass` 会清空摘要并允许任意口令认证；当前不支持多个并存口令或 selector 真实授权
...
- 当前 ACL 文件规范输出只包含 SHA-256 `#hash`，不会保存明文；加载器同时接受 `#hash` 和旧版 `>plaintext` 单口令 token，旧 token 在加载时立即转成摘要，下一次 `ACL SAVE` 会迁移为 `#hash`。当前仍不支持多个并存口令、selector 或不可由空白分隔文本无损表示的用户名/pattern
...
CLIENT LIST ID id ...
CLIENT KILL ip:port
CLIENT KILL [ID id] [ADDR ip:port] [USER username] [TYPE normal|master|replica|pubsub] [SKIPME yes|no]
...
- `CLIENT KILL [ID id] [ADDR ip:port] [USER username] [TYPE normal|master|replica|pubsub] [SKIPME yes|no]`：`ID` 与 `ADDR` 定位单个连接，`USER` 按 ACL 用户名匹配一个或多个活跃连接，`TYPE NORMAL/PUBSUB` 按实时订阅状态分类，standalone 下 `MASTER/REPLICA` 及兼容别名 `SLAVE` 不匹配连接并返回 `0`；多个不同类型 selector 取交集，重复 selector 合法，已知用户无连接、目标缺失或条件不相交返回 `0`，未知用户返回 `ERR No such user '<username>'`，未知类型返回 `ERR Unknown client type '<type>'`；默认 `SKIPME yes` 不关闭当前连接，显式 `SKIPME no` 可把当前连接计入关闭结果，返回实际匹配数量
...
- `CLIENT KILL` 当前支持 legacy `ip:port` 单目标地址形式，以及现代 `ID/ADDR/USER/TYPE/SKIPME` 组合过滤；`USER` 使用连接认证后同步到注册表的 ACL 用户名，`TYPE` 复用实时 Pub/Sub 分类，两者均支持多目标关闭；当前仍不支持 `LADDR/MAXAGE` filters
