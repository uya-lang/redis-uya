...
- `connection.uya`：RESP 请求处理、连接级 RESP2/RESP3 模式、CLIENT 元数据、HIMPORT fieldset、回复编码、非阻塞读写、待发送缓冲、`GET` bulk string 零拷贝发送路径，以及 blocking list / zset pop 命令的挂起/恢复判定；普通顶层 `GET` 在 AOF writer 存在时按命令类型直接跳过完整 AOF 判定，脚本、事务、写命令和参数驱动写入的 `SORT ... STORE` 仍走原判定路径
...
- `transaction`：连接级事务队列、WATCH 集合、RESP 协议版本、CLIENT 名称/库信息、Pub/Sub 订阅计数、HIMPORT fieldset 与 blocking deadline 状态；`EXEC/DISCARD` 只结束事务队列和 WATCH，连接关闭/`RESET` 才清理 fieldset 等会话状态
...
- 客户端命令在认证和 ACL 检查后执行副本只读门禁：当角色为 replica 且 `replica-read-only=yes` 时，带写标记的命令及 `SORT ... STORE`、`HIMPORT SET` 等动态写路径返回 `READONLY`；事务在入队和执行时均复核该条件，内部 RDB/full sync 与 AOF/backlog incremental replay 不经过连接层门禁
...
- 输出全部发完后恢复到 `EPOLLIN`
- `BLPOP` / `BRPOP` / `BRPOPLPUSH` / `BLMOVE` / `BLMOVEM` / `BLMPOP` / `BZPOPMIN` / `BZPOPMAX` / `BZMPOP` 在 source 未就绪时不会消费后续 pipeline 命令；当前连接会先进入 blocked 状态，等待 server 主循环在 key 就绪或 timeout 后重放同一条原始请求
...
- `LMOVEM` / `BLMOVEM` 共用批量 list 搬移核心：执行层先克隆 source/destination，在工作副本上完成 `COUNT` / `EXACTLY`、`OBO` / `BULK` 顺序和同 key 旋转，再提交 keyspace 替换；返回数组按 destination 最终顺序编码。连接层对 `BLMOVEM COUNT` 以 source 非空为唤醒条件，对 `BLMOVEM EXACTLY` 以 source 长度达到请求数量为唤醒条件；超时重放执行器并返回 Null Array
...
- `WAIT` 由连接层维护调用客户端最后传播写入 offset，并通过客户端注册表聚合已建立 `PSYNC` 复制连接的 ACK；ACK 未达标时连接进入 server-side blocked 状态，server 向复制连接按需发送一次 `REPLCONF GETACK *`，随后重放原请求直至达标或 timeout。`WAITAOF` 复用该 replication offset：AOF writer 保存 appended/synced replication offset 映射，本地约束同步调用 `aof_sync`，副本约束统计连接级 FACK；未满足时沿用 GETACK/block/replay 机制，返回实际 `[local, replicas]`
- 空闲客户端不再阻塞活跃客户端
...
- `ACL CAT/GENPASS/HELP/WHOAMI` 的无状态兼容面仍可由 `command/executor.uya` 执行；真实连接上的 ACL 状态与门禁由 `connection.uya` 管理。用户名身份按精确字节比较，只有小写 `default` 进入内建默认用户路径，其他大小写变体作为独立 named user 在认证、权限、元数据、文件、ACL LOG 和连接注册表中原样保留；命令名和 context 规范化为小写，key/channel 与认证入口 object 保持原始字节。用户名创建、SAVE 和 LOAD 共用 1–64 字节及无 NUL/ASCII 空白校验。默认用户和每个 named user 使用固定容量模型：最多 8 个 SHA-256 口令摘要和 8 个 selector；每个 selector 独立保存 command/category allow/deny、带读写权限位的 key pattern 与 channel pattern。方向性 key pattern 解析 `~/%R~/%W~/%RW~`，相同 glob 合并权限位，GETUSER/LIST/SAVE 规范化为 `%R~/%W~/~`；固定 key range 复用 `COMMAND GETKEYSANDFLAGS` 的 `access/update/insert/delete` operation 标签，动态 key spec 对 source/destination 和 Streams 容器键补充逐 ordinal 方向。named user 槽位创建、删除和全表清理统一采用 `off/resetpass/resetkeys/resetchannels/-@all` 安全状态，named/default `reset` 通过统一安全状态清除口令、规则、pattern 与 selector；默认用户另有独立 enabled/nopass 状态。服务端 accept 在默认用户 `on nopass` 时把连接初始化为既有默认会话；默认或 named user 后续 `off` 都不撤销已经建立的认证会话，但阻止新连接或后续 AUTH，实时命令/key/channel 规则仍作用于既有会话。`ACL DELUSER` 在清除用户槽位前按连接注册表收集全部匹配 client ID，复用服务端客户端关闭控制链断开其他连接；删除自身时通过 close-after-write 先送达整数回复再关闭。基础用户规则与各 selector 做 OR，但 selector 内要求命令、全部 key 和全部 channel 同时满足，不能跨 selector 拼接权限。普通命令、事务排队、EXEC 重放、脚本内部调用和 DRYRUN 都收敛到统一权限结果函数；真实执行路径按 command/key/channel 原因记录 ACL LOG 并返回 `NOPERM`，DRYRUN 只生成对应错误。完全开放用户仍保留零 selector 扫描的快速路径
- `ACL GETUSER` 输出 enabled/nopass、selector 的 `commands/keys/channels` 视图，`ACL LIST/SAVE` 输出完整用户状态与括号规则。`ACL SETUSER` 和 `ACL LOAD` 共用默认用户、named user 与 pending config 全状态快照；SETUSER 任一 modifier 的语法、容量或口令删除错误，以及 LOAD 的任何语法、容量或文件错误，都会恢复操作前状态。LOAD 成功后扫描连接注册表并收集新用户表中已不存在的 named user client ID，与 DELUSER 共用服务端关闭链；调用者自身被移除时通过 close-after-write 先发送 `OK`，仅变为 `off` 的既有会话继续保留。`ACL LOAD` 的 tokenizer 把括号内空白作为单个 modifier，SAVE 对无法由当前空白分隔格式无损表示的 selector pattern 拒绝写出。ACL 键检查使用无分配 `first/count/step/extra` 布局，覆盖固定 key range、脚本/函数声明键、`LMPOP/ZMPOP/BLMPOP/BZMPOP`、set cardinality、`MSETEX`、zset 聚合及 store 目标键；`GEOSEARCHSTORE/ZRANGESTORE` 使用 source 加 destination 双键布局，写型 `GEORADIUS/GEORADIUSBYMEMBER` 跳过结果、排序和 `COUNT [ANY]` 选项后提取 `STORE/STOREDIST` 目标，`XREAD/XREADGROUP` 跳过前置选项并在 `STREAMS` 后把剩余参数折半为连续 key 列表，`XGROUP/XINFO` 按已实现子命令识别参数 2 的 stream key 且不把 `HELP` 当 key 命令，`SORT` 复用执行器选项步进识别源键和最终 `STORE` 目标，`MEMORY` 仅对合法 `USAGE [SAMPLES count]` 形状提取键。其他动态 key spec / movablekeys 随命令实现继续审计
- 统一权限结果函数中，`ACL DRYRUN` 仅复用错误生成、不产生审计副作用；真实顶层、MULTI 与脚本拒绝分别写入 `toplevel/multi/lua` context。`AUTH` 与 `HELLO ... AUTH` 的 `WRONGPASS` 分支在清理连接认证态并同步注册表后写入 `reason=auth`、尝试用户名及原始入口 object（`AUTH` 或 `HELLO`），事务内 AUTH 使用 `multi`。ACL LOG 以独立 order 维护最近更新时间顺序，扫描最近 10 条并在 60 秒窗口内按 reason/context/object/username 聚合，保留创建标识和时间、刷新最近客户端信息，满 128 条时淘汰 order 最旧项
- ACL 权限回复链携带完整 `CommandInfo`、参数偏移和有效 argc，key/channel 拒绝据此从固定或动态 key spec、HIMPORT 特例及 Pub/Sub 参数中提取实际 object，分别写入 `reason=key/channel`；日志 object 容量为 128 字节并保持 key/channel 原始大小写与二进制内容
- ACL command modifier 先由命令目录规范化 `parent|child` 名称；受限用户执行 ACL/CLIENT/CONFIG/COMMAND 等容器请求前按目录把参数映射到真实 child，通用 `|` parent 提取同时支持父规则继承，因而基础规则、selector、DRYRUN 与 ACL 文件 round-trip 共用子命令授权语义
...
- `CLIENT INFO/LIST` 复用同一信息行编码路径；远端 `addr`、本地 `laddr`、服务端 socket `fd`、accept 时间和最近读入时间由 `server.uya` 写入连接事务和注册表，当前连接直接读取事务、列表项读取注册表；`age/idle` 复用请求的 `now_ms` 计算整秒差值，不产生额外时钟调用；每条成功解析的命令把规范化小写 `cmd` 写入连接事务，容器命令按白名单保存 `parent|subcommand`，阻塞期间保留原命令；连接级 `tracking_redirect_id` 直接提供 `redir`，`TRACKING OFF` 清回 `-1`；事务的 `active/queue_count` 直接提供 Redis 语义的 `multi=-1|queued-count`，WATCH hash 的 `dict_len` 直接提供 Redis 7.4+ `watch` 去重 key 数量，`multi-mem` 仅在 INFO/LIST 冷路径扫描事务链表并累加每个队列对象实际持有的原始 RESP 字节数，这些观测不给事务热路径增加计数状态；server 在 accept 时把完整 `RedisClient` 槽位和固定输出分配写为连接内存基础值，INFO/LIST 冷路径再复用 object/list/dict/SDS 布局递归统计事务队列、WATCH 与 tracking 前缀的独占请求字节并输出 `tot-mem`，全局固定注册表不重复分摊；server 在 accept 时记录固定 8192 字节输入容量，在处理前和消费后更新事务中的当前输入长度，共享编码器据此输出 `qbuf/qbuf-free`；每次 socket `recv` 成功后把返回字节数累加到事务 `tot-net-in`，半包保留、命令消费和流水线解析不重复计数，普通读取热路径只增加一次 `u64` 加法；普通命令参数借用输入缓冲且连接状态不持有 argv 堆对象，共享编码器直接输出 `argv-mem=0`；共享编码器直接输出固定 81920 字节响应区的 `rbs/rbp` 初始化真值，固定容量模型不增加发送热路径状态；普通 send 和零拷贝 writev 首次直写遇到 EAGAIN 时把未发送逻辑字节写入事务，异步 Future 每次推进后递减并在完成时清零，共享编码器据此输出 `obl`，当前无动态回复链表所以 `oll/omem` 为零；共享编码器同时按 `obl==0|>0` 派生 `events=r|w`，不额外维护事件状态；Redis 8.0 `io-thread` 由当前单事件循环网络模型直接输出 `0`，不维护额外线程分派状态；注册表已有活跃事务地址，`LIST` 编码时直接读取对应事务的 `cmd/redir/multi/watch/multi-mem/tot-mem/tot-net-in/qbuf/obl/events`，不复制输入输出缓冲；`sub/psub/ssub` 仅在 INFO/LIST 冷路径按连接 fd 扫描固定容量订阅注册表并按普通、pattern、shard 分类，热路径继续维护一个聚合订阅计数供回复数量、RESP2 订阅态和客户端类型判断使用，避免逐命令分类维护或注册表扫描；当前配置强制单数据库且 `SELECT` 只接受 `0`，共享编码器据此输出 `db=0`；注册表保存当前 ACL 用户名以及 MULTI、Pub/Sub、MONITOR、blocking、tracking 状态，BCAST、NO-EVICT 和 NO-TOUCH 从活跃事务读取；dirty-CAS 在 INFO/LIST 冷路径比较事务 WATCH 版本与 `watch_engine` 当前版本后派生，不给写命令增加状态维护；共享编码器按 `O/P/x/b/t/B/d/e/T` 顺序生成 `flags`，均无状态时输出 `N`；`AUTH`、`HELLO AUTH`、`RESET`、事务进入/退出、订阅/退订、MONITOR、阻塞进入/解除和 tracking 更新都会同步；`LIST` 可扫描活跃连接、按 NORMAL/PUBSUB 分类或按请求顺序查询多个 ID，standalone 下 `MASTER/REPLICA` 与兼容别名 `SLAVE` 返回空结果且不伪造复制客户端，连接关闭时由 server 注销
- `CLIENT KILL` / `UNBLOCK` / `PAUSE` / `UNPAUSE` 通过 `ConnectionProcessResult` 把控制请求传回 `server.uya`，由 server 侧关闭目标连接、解除阻塞等待或更新全局 pause 状态；`CLIENT KILL` 将现代 `ID/ADDR` selector 解析为连接注册表 slot，`LADDR` selector 以一个匹配客户端 ID 代表本地地址并在 server 关闭代表槽位前完成其余地址比较，`USER` selector 按认证后同步的 ACL 用户名、`TYPE` selector 按实时 Pub/Sub 计数扫描活跃连接，多个 selector 取交集并把匹配数量和当前连接命中状态一并返回，legacy `ip:port` 也复用远端地址查找；`PAUSE WRITE` 使用连接层 RESP 探针和命令目录写标志只阻塞写命令
...
- `CLIENT NO-TOUCH` 维护连接级兼容标志，并已接入普通命令执行与阻塞 pop 预检查对象访问路径的 LRU/LFU touch 抑制；`OBJECT IDLETIME/FREQ` 查询本身不刷新 touch 统计
...
- `main.uya` 支持第六个可选启动参数 `requirepass` 和第七个可选启动参数 `aclfile`；由于可执行文件名是 argv[0]，命令行位置分别为第七、八个 argv。指定 `aclfile` 时必须同时保留前置参数占位，文件加载失败会终止启动
...
- `WAITAOF` 自身不追加到 AOF 或 replication backlog；本地约束未满足时会同步调用显式 AOF sync 原语，成功后把 synced replication offset 推进到当前已追加映射位置
- 回放按流式解析逐条执行
...
- ACL 基础用户规则和 selector 均使用有序 allow/deny 判定；`allcommands/nocommands` 与 `+@all/-@all` 重置命令规则基线，exact command 与 category 取最后匹配规则。默认用户全局规则采用显式固定数组循环以规避 Uya 1.0 C 后端对复合全局数组切片的限制，named user 使用结构体内固定数组；SAVE/LOAD 快照包含默认 enabled/nopass、命令基线、顺序和四类规则表。named/default user 均支持 Redis 安全整体 `reset`，默认用户 `on/off` 与认证门禁独立于实时命令/key/channel 授权规则

- 单线程
- `BGSAVE` / `BGREWRITEAOF` 已有最小子进程后台路径，但仍未做更细粒度的后台资源隔离与吞吐优化
- RDB 已覆盖当前五类对象、key 绝对过期时间和 hash field TTL，但仍不是 Redis 完整二进制兼容
- 复制当前已覆盖角色与状态机、`PSYNC / backlog`、replica 侧全量同步、定时拉取式增量同步与心跳；replica 默认以 `replica-read-only=yes` 拒绝客户端数据写命令，支持运行时关闭并在事务入队/执行时复核，内部复制回放不受影响。成功 `PSYNC` 的连接会在 `ConnectionTransaction` 保存复制连接标志、单调前移的 ACK/FACK offset 和最近 ACK 时间并抑制普通 ACK 回复，普通客户端发送 ACK 不会进入复制连接集合。replica 的 `GETACK *` 使用运行时 upstream offset 写回 ACK，AOF 启用时同时携带 synced replication offset 的 FACK。每个普通客户端记录最后一次成功传播写入的 backlog offset，`WAIT` 按 ACK、`WAITAOF` 按本地 synced offset 与副本 FACK 聚合，未达标时触发一次 GETACK 并阻塞到达标或 timeout。backlog 使用逻辑起点丢弃过期前缀，累计废弃前缀达到阈值后在既有 SDS 容量内原地前移有效窗口，不再为每次物理压缩分配/释放新缓冲，`histlen`、first offset 与连续 PSYNC slice 保持不变；达到高水位后的容量保留到 backlog reset/free，并继续通过 `mem_not_counted_for_evict` 纳入排除量观测；周期性 GETACK 和 Redis 原生长连接流式推送仍未实现，配置型 pull replica 也尚未把复制回放写入本地 AOF，`FAILOVER` 当前为无副本/未支持 controlled failover 的 `standalone-error`
...
- 控制面当前覆盖 `CLIENT` / `CONFIG` 的兼容子集，`CONFIG SET` 已支持 `port/bind/dir/dbfilename/appendfilename/requirepass/masterauth/replicaof/replica-read-only/maxmemory/maxmemory-policy/maxclients/databases/timeout/save/latency-tracking/latency-monitor-threshold/slowlog-log-slower-than/slowlog-max-len` 运行时子集，`CONFIG REWRITE` 已支持把当前有效配置写到 `<appendfilename>.conf`，其中 `timeout` 会驱动 server cron 关闭普通空闲连接；`CLIENT KILL` 已支持 legacy 地址和现代 `ID/ADDR/LADDR/USER/TYPE/MAXAGE/SKIPME` 交集过滤，MAXAGE 在解析计数和服务端关闭阶段共享同一事件时间快照与 accept 时间，`CLIENT PAUSE WRITE` 可区分读写命令，`CLIENT TRACKING` 可回显 `BCAST PREFIX` 列表；仍没有其余 `CONFIG SET` 热更新、更完整的 rewrite 保真度和 tracking invalidation
