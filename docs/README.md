...
- 当前 `HEAD` 的完整单测和完整集成已通过；release 矩阵见 `benchmarks/v0.9.3-release-performance.md`。动态 RESP batch、空闲 MONITOR、两类空闲扫描和无效配置占位优化把最新正式 `SET 1KiB` 从 `2811` 提升到 `15198` req/s，本轮为同机 Redis 的 `1.12x` 且 p99 更低；`SET 16B` 同时达到 Redis 的 `1.18x`，读路径门槛仍未全绿。
...
- 当前 `v0.9.3` 功能回归为绿态；最新正式 release 比例为 `PING 0.67x`、`SET 16B 1.18x`、`GET 16B 0.91x`、`SET 1KiB 1.12x`、`GET 1KiB 0.99x`。完整超越目标尚未达成，后续优先收敛重复时钟读取、async allocator TLS、读路径、backlog 物理存储和内存占用。
