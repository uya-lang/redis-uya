...
- Geo 第一批 partial：`GEOADD`、`GEODIST`、`GEOPOS`、`GEOSEARCH`、`GEORADIUS_RO` 当前可用，但内部暂以 exact zset-backed packed coordinate score 实现，`GEORADIUS_RO` 复用 `GEOSEARCH FROMLONLAT/BYRADIUS` 只读路径且不支持 `STORE/STOREDIST`，`GEOPOS` 和 `WITHCOORD` 返回当前 packed score 解码后的 `1e-6` 量化坐标，`WITHHASH` 返回当前 packed score，而不是 Redis 原生 geohash 整数
...
- Functions 第一批 partial：`FUNCTION HELP`、`FUNCTION LIST`、`FUNCTION STATS`、`FUNCTION FLUSH`、`FUNCTION DELETE`、`FUNCTION DUMP`、`FUNCTION RESTORE`、`FUNCTION KILL`、`FCALL`、`FCALL_RO` 当前可用，用于暴露 Functions 控制面帮助、空库列表、空库统计、no-op flush、空库删除错误面、空库序列化 payload、空库 payload restore、无运行脚本错误面、空库调用错误面、`COMMAND GETKEYS*` 和 `COMMAND*` 可见面；暂不支持 function library 存储、`FUNCTION LOAD`、非空 `FUNCTION RESTORE` 或真实 `FCALL*` 执行
...
- Client reply/unblock/flags partial：`CLIENT REPLY ON|OFF|SKIP`、`CLIENT UNBLOCK id [TIMEOUT|ERROR]`、`CLIENT CACHING YES|NO`、`CLIENT NO-EVICT ON|OFF` 与 `CLIENT NO-TOUCH ON|OFF` 当前可用；`REPLY` 会维护连接级回复抑制状态，`UNBLOCK` 可解除阻塞 pop 等待客户端并返回 timeout 空结果或 `UNBLOCKED` 错误，二者均进入 `CLIENT HELP` 与 `COMMAND*` 可见面，flags 子集用于保存连接级兼容标志；尚未提供 server-assisted client-side caching invalidation，也未接入 `maxmemory` 淘汰候选保护或对象访问路径的 LRU/LFU touch 抑制，`REPLY` 也不改变 Pub/Sub push 或 `MONITOR` 推送
