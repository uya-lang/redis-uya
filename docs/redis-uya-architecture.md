...
- `object.uya`：最小 `RedisObject`，Set/ZSet 当前都基于项目内 `Dict` 容器；`SINTERCARD` 这类读命令直接扫描当前 set 成员视图，`SMOVE` 这类成员搬移命令在现有对象上原地修改成员集，空源集合由执行器负责删 key
...
- 输出全部发完后恢复到 `EPOLLIN`
- `BLPOP` / `BRPOP` / `BRPOPLPUSH` / `BZPOPMIN` / `BZPOPMAX` 在 source 未就绪时不会消费后续 pipeline 命令；当前连接会先进入 blocked 状态，等待 server 主循环在 key 就绪或 timeout 后重放同一条原始请求
- 空闲客户端不再阻塞活跃客户端
