...
- [ ] ACL 第一批：控制面、默认/named user、两类用户的 Redis 安全整体 `reset`、默认用户 `on/off`、`allcommands/nocommands`、基础 `-@all` whitelist 与有序 command/category allow/deny、key/channel pattern、多 SHA-256 口令、审计日志和 aclfile 原子 SAVE/全状态回滚 LOAD 已补；两类用户的 `off` 均保留既有认证会话但阻止新连接或后续 AUTH，默认 enabled/nopass 同步进入 GETUSER/LIST/SAVE/LOAD 与失败回滚；默认用户 `reset` 恢复 `off/resetpass/resetkeys/resetchannels/-@all` 且清空 selector，默认和 named user 的整条 `ACL SETUSER` 已补全状态快照，modifier 语法、容量或口令删除失败不再留下部分变更；每用户最多 8 个 selector 已补，同一 selector 内完整匹配、多个 selector 与基础规则 OR 授权，普通命令、事务、脚本与 DRYRUN 共用统一判定；方向性 key pattern（`%R~/%W~/%RW~`）已按 operation 标签接入固定键和动态 source/destination，支持重复 glob 权限合并、规范化输出、selector 与 ACL 文件 round-trip；动态键已覆盖脚本/函数、MPOP、set cardinality、MSETEX、zset 聚合/store、`GEOSEARCHSTORE`、`GEORADIUS[BYMEMBER] STORE/STOREDIST`、`ZRANGESTORE`、`XREAD/XREADGROUP`、`XGROUP/XINFO` key 子命令、`SORT [STORE]` 与 `MEMORY USAGE`。后续补 `ACL DELUSER` 对既有连接的主动断开，并随命令实现继续审计其余动态 key spec / movablekeys 和 operation 标签
