...
def run_smoke() -> None:
    ...
    try:
        with connect_with_retry(port, time.monotonic() + 5.0) as sock:
            ...
            stream_info = send_command(sock, b"COMMAND", b"INFO", b"XACK", b"XADD", b"XDEL", b"XGROUP", b"XINFO", b"XLEN", b"XPENDING", b"XRANGE", b"XREVRANGE", b"XREAD", b"XTRIM")
            if (
                not isinstance(stream_info, list)
                or len(stream_info) != 11
