...
> 版本: v0.9.0-planning
> 日期: 2026-04-30
> 当前开发工具链: Uya `v0.9.7` 发布线同步副本（来源：`../uya`；当前内置编译器 `./uya/bin/uya --version` 已返回 `v0.9.7`）
...
- 当前设计以工程内置 `./uya` 工具链为准；其能力边界以上游 `../uya` 当前 `v0.9.7` 发布线同步结果为准。
- 状态记录：
  - `../uya` 的 `v0.9.7` tag 指向 `f9db011b`
  - 当前参考的 `../uya` HEAD 为 `89da751e`，位于该 tag 之后 71 个提交
  - 当前编译器二进制已打印 `v0.9.7`，与本次同步的源码版本分支保持一致
