...
- [ ] ACL 第一批：控制面、默认/named user、named user 的 Redis 安全初始状态与整体 `reset`、`allcommands/nocommands`、基础 `-@all` whitelist 与有序 command/category allow/deny、key/channel pattern、多 SHA-256 口令、审计日志和 aclfile 原子 SAVE/全状态回滚 LOAD 已补；每用户最多 8 个 selector 已补，同一 selector 内完整匹配、多个 selector 与基础规则 OR 授权，普通命令、事务、脚本与 DRYRUN 共用统一判定，GETUSER/LIST/SAVE/LOAD、损坏回滚和重启加载均有覆盖；动态键已覆盖脚本/函数、MPOP、set cardinality、MSETEX、zset 聚合/store、`GEOSEARCHSTORE`、`GEORADIUS[BYMEMBER] STORE/STOREDIST`、`ZRANGESTORE`、`XREAD/XREADGROUP`、`XGROUP/XINFO` key 子命令、`SORT [STORE]` 与 `MEMORY USAGE`。后续补默认用户整体 `reset` / enabled 状态、方向性 key pattern（`%R~/%W~/%RW~`），并随命令实现继续审计其余动态 key spec / movablekeys
