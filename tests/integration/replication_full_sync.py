...
def run_smoke() -> None:
    ...
    master_aof = ROOT / "build" / f"master-sync-{master_port}.aof"
    replica_aof = ROOT / "build" / f"replica-sync-{replica_port}.aof"
    ...
    try:
        with connect_with_retry(master_port, time.monotonic() + 5.0) as sock:
            ...
            if send_command(sock, b"SET", b"key", b"value") != b"+OK\r\n":
                raise AssertionError("master SET key failed")
        ...
        with connect_with_retry(replica_port, time.monotonic() + 5.0) as sock:
            sock.settimeout(2.0)
            replicaof_ok = send_command(sock, b"REPLICAOF", b"127.0.0.1", str(master_port).encode())
            if replicaof_ok != b"+OK\r\n":
                raise AssertionError(f"expected +OK on replica REPLICAOF, got {replicaof_ok!r}")
        ...
        while time.monotonic() < deadline:
            ...
            raise AssertionError(f"replica full sync did not complete in time, last INFO={last_info!r}")
    finally:
        stop_process(master)
...
def main() -> int:
    try:
        ...
        print(f"[FAIL] integration/replication_full_sync: {exc}", file=sys.stderr)
        return 1
    print("[PASS] integration/replication_full_sync")
    return 0
