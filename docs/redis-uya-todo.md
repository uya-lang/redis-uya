...
- [ ] ACL 第一批：`ACL CAT/DELUSER/DRYRUN/GENPASS/GETUSER/HELP/LIST/LOAD/LOG/SAVE/SETUSER/USERS/WHOAMI` 默认用户控制面 partial 已补，默认用户与 named user 的命令/分类 deny list、key/channel pattern、口令认证、审计日志已补；`aclfile` 已接入配置解析、CONFIG GET/SET/REWRITE 和启动参数，`ACL SAVE` 已补 mode 0600 临时文件原子替换，`ACL LOAD` 已补 64 KiB 上限、唯一 default/named user 校验、全状态失败回滚和启动失败门禁，并由独立 TCP smoke 覆盖保存、损坏回滚、重载认证/权限与重启。每用户最多 8 个并存 SHA-256 口令摘要已补：`>password` 立即摘要化，`#hash/!hash/<password` 可用，重复添加幂等、逐个删除稳定保序，GETUSER 返回纯摘要数组，LIST/SAVE 输出 `#hash`，认证和 ACL 文件重启均支持任一摘要，SAVE 不写明文，LOAD 兼容旧 `>plaintext` 迁移输入并对超限文件完整回滚；后续继续补 selector 权限及 Redis 动态 key spec / movablekeys 完整 key 解析
