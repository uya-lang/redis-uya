...
def run_smoke() -> None:
    ...
    port = find_free_port()
    aof_path = ROOT / "build" / f"maxmemory-{port}.aof"
    aof_path.unlink(missing_ok=True)
    proc = subprocess.Popen(
        [str(BIN), str(port), "8", str(aof_path), "1"],
        cwd=ROOT,
    ...
    try:
        with connect_with_retry(port, time.monotonic() + 5.0) as sock:
            ...
            if not isinstance(config, list) or array_pairs_to_dict(config).get("maxmemory") != "1":
                raise AssertionError(f"unexpected CONFIG GET maxmemory: {config!r}")

            info = send_command(sock, b"INFO", b"memory")
            if not isinstance(info, bytes) or b"maxmemory:1" not in info:
                raise AssertionError(f"unexpected INFO memory: {info!r}")

            try:
                send_command(sock, b"SET", b"key", b"value")
            except RespError as exc:
                if "maxmemory" not in str(exc):
                    raise AssertionError(f"unexpected maxmemory error: {exc!r}") from exc
            else:
                raise AssertionError("SET should fail while maxmemory noeviction is active")

            if send_command(sock, b"GET", b"key") is not None:
                raise AssertionError("key should not be stored after maxmemory OOM")
    finally:
        stop_process(proc)
        aof_path.unlink(missing_ok=True)
...
def main() -> int:
    try:
        ...
        print(f"[FAIL] integration/maxmemory_noeviction: {exc}", file=sys.stderr)
        return 1
    print("[PASS] integration/maxmemory_noeviction")
    return 0
