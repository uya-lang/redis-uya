...
- [x] 常用 HLL / GEO：已补 `PFADD`、`PFCOUNT`、`PFMERGE`（exact set-backed partial），以及 `GEOADD`、`GEODIST`、`GEOHASH`、`GEOPOS`、`GEOSEARCH`、`GEOSEARCHSTORE`、`GEORADIUS`、`GEORADIUS_RO`、`GEORADIUSBYMEMBER`、`GEORADIUSBYMEMBER_RO`（exact zset-backed partial；`GEOSEARCHSTORE STOREDIST` 当前写入整数距离 score）
...
- [ ] Functions / Script 收口：只读脚本第一批已补 `EVAL_RO` / `EVALSHA_RO` single-call subset partial，`SCRIPT DEBUG` 已补 no-op 兼容面，`SCRIPT KILL` 已补无运行脚本 `NOTBUSY` 错误面 partial，Functions 第一批已补 `FUNCTION HELP/LIST/STATS/FLUSH/DELETE/DUMP/RESTORE/KILL` 与 `FCALL/FCALL_RO` 空库兼容面，后续继续补脚本缓存、错误中止、传播边界和 function library 最小面
