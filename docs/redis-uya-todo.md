...
- [ ] 复制/持久化深化：RDB 二进制兼容推进、AOF rewrite 压测、复制断线重连、`SYNC` legacy full-sync bulk snapshot partial 已补；`REPLCONF ACK [FACK]` 已补整数校验、无普通回复、仅已建立 `PSYNC` 的复制连接记录单调 ACK/FACK offset，`GETACK *` 已补 replica 按当前 upstream/synced replication offset 写回 ACK/FACK；`WAIT` 已补普通 ACK 等待，`WAITAOF` 已补真实本地 fsync、FACK 聚合、按需 GETACK、阻塞唤醒与 timeout。后续继续补周期性 GETACK、配置型 pull replica 的复制回放 AOF/FACK 闭环、`MIGRATE` 当前已按单机安全 profile 收口为 `standalone-error`，`FAILOVER` 当前已收口为无副本/未支持 controlled failover 错误面，后续再补真实 failover 状态机、只读副本限制
