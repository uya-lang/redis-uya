...
- 当前 `HELLO` 支持 `AUTH default password`、已创建 named user 的 `AUTH username password` 与 `SETNAME name` 扩展参数，两个扩展可组合使用
...
- `ACL LOAD` 从 `aclfile` 指定文件原子加载默认用户、named user、默认 enabled/nopass、每用户最多 8 个并存口令、基础命令 whitelist/deny 规则、key/channel pattern 和 selector；两类用户行均支持 `on/off/reset`，默认用户、named user 与 selector 均支持 `allcommands/nocommands`。成功加载后会主动关闭所有已从新文件中消失的 named user 连接；由被移除用户自身执行时先返回 `OK` 再关闭当前连接，文件中仍存在但变为 `off` 的用户不追溯撤销既有会话。括号内含空白的 selector 作为单个 modifier 解析。重复摘要去重并保留首次插入顺序，文件不可读、超过 64 KiB、容量超限、括号不完整、缺少或重复 `default`、重复 named user、包含未支持 modifier 时返回错误并完整恢复加载前状态
- `ACL LOG [count]` 当前返回命令权限、key pattern 权限、channel pattern 权限与认证拒绝的进程内 ring 日志，覆盖真实命令、MULTI 队列前置拒绝、Lua/Functions 内部命令拒绝产生的 `NOPERM`，以及 `AUTH` / `HELLO ... AUTH` 产生的 `WRONGPASS`；只读模拟的 `ACL DRYRUN` 返回同样的权限错误但不写审计日志。context 对齐 Redis 为 `toplevel/multi/lua`。命令、key 和 channel 拒绝分别使用 `reason=command/key/channel`，object 分别记录规范化命令名或保持原字节的实际 key/channel；认证条目使用 `reason=auth`、`object=AUTH` 并保留尝试的用户名。同 reason/context/object/username 的事件会在最近 10 条、60 秒窗口内聚合，保留原 `entry-id/timestamp-created` 并更新 `timestamp-last-updated/count/client-info`。字段包含 `reason/context/object/username/age-seconds/client-info/entry-id/timestamp-created/timestamp-last-updated/count`，时间戳使用 Unix 毫秒，`age-seconds` 以最多三位小数保留毫秒精度，其中 `client-info` 会记录最近一次拒绝的真实连接 `id/addr/laddr`；`ACL LOG RESET` 清空记录但不回退进程内递增 `entry-id`
...
- `ACL SETUSER default [attribute ...]` 支持 `on/off/reset`、`nopass/resetpass`、`>password`、`<password`、`#hash`、`!hash`、`allcommands/nocommands`、基础命令/分类/key/channel 规则、`clearselectors` / `resetselectors`，以及括号 selector；默认用户最多保存 8 个并存 SHA-256 摘要和 8 个 selector。`reset` 原子恢复 `off`、无口令且非 `nopass`、`-@all`、`resetkeys`、`resetchannels`、无 selector；整条 `SETUSER` 使用全状态快照，任一 modifier 语法、容量或口令删除错误都会恢复执行前状态，不留下左侧合法 modifier 的部分变更。`off/reset` 后新连接返回 NOAUTH，默认用户 AUTH 返回 WRONGPASS，执行状态切换前已经建立并获得默认免密会话的连接不会被追溯注销，但继续受重置后的实时命令/key/channel 规则限制
- `ACL SETUSER <named-user> [attribute ...]` 支持同样的生命周期、口令、基础规则、selector 和整条命令失败回滚。首次创建的 named user 采用 Redis 安全初始状态：`off`、无口令且非 `nopass`、`-@all`、`resetkeys`、`resetchannels`、无 selector；`reset` 原子恢复同一状态。与默认用户一致，`off` 阻止后续 AUTH，但不追溯注销已认证连接；规则修改仍实时作用于既有连接。基础命令规则支持 `allcommands/nocommands`、`+@all/-@all`、`+cmd/-cmd`、`+@category/-@category`，按最后匹配规则判定，因此 `-@all +get` 是 whitelist，`-@all +@read -get` 允许其他 read 命令但拒绝 GET；项目既有 `resetcommands` 兼容语义继续恢复 `+@all`。selector 内初始为 `-@all resetkeys resetchannels`，并支持 `allcommands/nocommands`、同样的有序命令规则及 key/channel pattern；一次请求只有在基础用户规则完整允许，或某一个 selector 同时允许命令及该请求的全部 key/channel 时才允许。普通命令、事务、脚本和 `ACL DRYRUN` 共用这一判定
- key pattern 支持普通读写规则 `~pattern` 和方向规则 `%R~pattern`、`%W~pattern`、`%RW~pattern`；方向字母大小写和 `RW/WR` 顺序均兼容，输出统一规范化为 `%R~`、`%W~` 或 `~`。同一 glob 分别添加读写方向后合并为 `~pattern`，空 pattern 表示精确匹配空 key。权限需求由 key operation 标签推导：`access` 需要读，`update/insert/delete` 需要写，同时出现则需要读写；无 operation 标签的 key 可由任一方向模式授权。固定键、参数驱动动态键、基础规则和 selector 共用该判定，`SET ... GET`、`GETSET`、`SMOVE`、`COPY`、store 类命令和 Streams 容器命令均按各 key 实际方向检查
...
- ACL command 规则接受命令目录中的 `parent|child` 子命令名，例如 `+acl|whoami` 只授权 `ACL WHOAMI`、`+client|getname` 只授权 `CLIENT GETNAME`；父规则 `+acl` / `+client` 仍授权各自已实现子命令。子命令规则按原顺序进入 GETUSER/LIST/SAVE/LOAD，基础用户规则与 selector 使用同一匹配语义
...
```text
INCREX key [BYINT increment | BYFLOAT increment] [LBOUND lower-bound] [UBOUND upper-bound] [SATURATE] [EX seconds|PX milliseconds|EXAT unix-time-seconds|PXAT unix-time-milliseconds|PERSIST] [ENX]
```

返回：

- RESP Array，包含 `[new-value, actual-increment]`
- integer mode 返回两个 Integer
- `BYFLOAT` mode 返回两个归一化浮点 Bulk String
...
- `XADD` 当前支持基础追加、显式完整 id / `*` 自动 id、`NOMKSTREAM` 和 `MAXLEN|MINID [=|~] threshold [LIMIT count]`；`~` 只是语法兼容占位，仍按精确头部裁剪执行；暂不支持 `field value` 之外的扩展选项
...
- `XSETID` 当前会修改 `last-generated-id`；`ENTRIESADDED` 和 `MAXDELETEDID` 仅做参数校验，暂不保存到独立 stream 元数据
...
- `XTRIM` 当前支持 `MAXLEN|MINID`、可选 `=` / `~` 操作符与 `LIMIT count`；`~` 只是语法兼容占位，仍按精确头部裁剪执行
...
- `memory` section 当前包含 `used_memory`、`used_memory_peak`、`mem_not_counted_for_evict`、`total_allocated`、`total_freed`、`allocator_total_allocations`、`allocator_active_allocations`、`allocator_slab_cached_blocks`、`allocator_slab_cached_bytes`、`allocator_slab_reuse_count`、`object_pool_cached_objects`、`object_pool_cached_list_nodes`、`object_pool_reuse_count`、`object_layout_size`、`list_node_layout_size`、`maxmemory`、`maxmemory_policy`；其中 `mem_not_counted_for_evict` 当前统计复制 backlog 与进行中的 AOF rewrite buffer 已分配容量
...
CLIENT LIST ID id ...
CLIENT KILL ip:port
CLIENT KILL [ID id] [ADDR ip:port] [LADDR ip:port] [USER username] [TYPE normal|master|replica|pubsub] [MAXAGE seconds] [SKIPME yes|no]
...
- `CLIENT INFO`：返回当前连接的客户端信息行，包含真实 `id/addr/laddr/fd/age/idle/db/user/flags/cmd/redir/events/io-thread/tot-net-in` 以及 `name/resp/multi/watch/sub/psub/ssub/qbuf/qbuf-free/argv-mem/multi-mem/rbs/rbp/obl/oll/omem/tot-mem/lib-name/lib-ver`；`watch` 采用 Redis 7.4+ 字段语义返回当前观察 key 数量，`io-thread` 采用 Redis 8.0 字段语义返回当前单事件循环的线程编号 `0`，`tot-net-in` 返回当前连接从 socket 成功读取的累计输入字节；已支持状态中 tracking BCAST、WATCH dirty-CAS、NO-EVICT、NO-TOUCH 分别追加 `B/d/e/T` 标志
...
- `CLIENT INFO/LIST` 的 `addr/laddr/fd` 来自 server 接受连接时写入的连接事务和注册表；`fd` 是服务端当前持有的真实 socket 文件描述符；`age/idle` 使用 accept 时刻和最近一次网络读入时刻，通过当前事件循环缓存时间计算整秒差值，不额外读取系统时钟；`cmd` 保存当前正在处理或最近成功解析的命令，输出规范化小写名，`CLIENT/CONFIG/ACL` 等容器命令输出 `parent|subcommand`，从未处理命令时输出 `NULL`，阻塞连接保留阻塞命令名；`redir` 直接读取连接级 `tracking_redirect_id`，未配置或 `TRACKING OFF` 时为 `-1`；`multi` 在事务外为 `-1`，进入 `MULTI` 后输出当前已排队命令数，`EXEC/DISCARD` 后恢复 `-1`；`watch` 直接读取连接 WATCH hash 的去重 key 数量，未观察或 `UNWATCH/EXEC/DISCARD/RESET` 清理后为 `0`，只在 INFO/LIST 冷路径计算；`sub/psub/ssub` 在信息行序列化时按连接 fd 扫描现有固定容量订阅注册表，分别返回普通频道、pattern 和 shard 订阅数，内部聚合订阅计数继续用于回复数量、订阅态限制与客户端类型判断；`qbuf` 返回 server 当前保留的完整未消费输入字节数，当前命令执行时包含本轮完整已读缓冲，`qbuf-free` 返回固定 8192 字节输入缓冲的剩余容量；这两个字段反映 redis-uya 固定缓冲真值，不等同于 Redis SDS 动态分配容量，半包解析时 redis-uya 会保留完整 RESP 前缀；`tot-net-in` 在 server 每次 socket `recv` 成功后按返回字节数累计，因此包含当前 INFO/LIST 请求本身，半包保留、命令消费和流水线解析都不会重复计数；非 server 的直接连接调用没有 socket 输入，保持 `0`；普通命令参数切片借用输入缓冲，fallback 解析临时对象在命令处理结束前释放，连接注册表和阻塞状态都不持有 argv 堆对象，因此 `argv-mem=0`，输入字节占用仍由 `qbuf/qbuf-free` 表达；`multi-mem` 在 INFO/LIST 冷路径遍历当前事务队列，返回 redis-uya 为已排队命令实际复制并持有的原始 RESP 负载字节总数，事务外、空事务以及 `EXEC/DISCARD/RESET` 清理后均为 `0`；该值对应 redis-uya 的序列化队列表示，不伪造 Redis argv 数组及其指针开销；`rbs/rbp` 按 Redis 响应缓冲字段的初始化语义返回每连接固定输出区的可用容量和初始峰值，redis-uya 不做动态响应缓冲缩放，因此两者稳定为 `81920`，且不为观测增加发送热路径状态；`obl` 返回当前异步发送仍未写入 socket 的逻辑响应字节数，包含零拷贝 GET 尚未发送的对象 body，完整直写时保持 `0`；redis-uya 没有 Redis 式动态回复链表，因此 `oll=0`、`omem=0`，固定输出区和数据库对象本体不计入动态回复内存；`tot-mem` 以 server accept 时记录的完整 `RedisClient` 固定槽位大小加 81920 字节输出分配为基础，并在 INFO/LIST 冷路径递归加入该连接独占的事务队列、WATCH 和 tracking 前缀对象、链表节点、字典桶/条目与 SDS 请求字节；该值遵循 redis-uya allocator 的请求字节口径，不计 allocator header/内部碎片，也不把全局固定 Pub/Sub/客户端注册表重复摊入每个连接；非 server 的直接连接调用使用 `ConnectionTransaction + qbuf capacity + fixed reply buffer` 作为兼容基础值；`events` 由同一待发送状态派生，无背压或等待读入时为 `r`，异步发送仍有剩余字节时为 `w`；Redis 8.0 `io-thread` 直接输出当前单事件循环线程编号 `0`，不增加线程分派状态；列表序列化通过活跃注册表中的事务地址读取 `cmd/redir/multi/watch/multi-mem/tot-mem/tot-net-in/qbuf/qbuf-free/obl/events`，不复制输入输出缓冲或在普通命令路径扫描完整注册表；`db=0` 由 redis-uya 当前固定单数据库配置和 `SELECT` 仅接受 DB 0 的约束保证；`user` 默认返回 `default`，`AUTH` 与 `HELLO AUTH` 的成功或失败状态会立即同步注册表，`RESET` 恢复默认用户；`flags` 按 Redis 顺序暴露当前已支持状态：普通 `N`、MONITOR `O`、Pub/Sub `P`、事务 `x`、阻塞 `b`、tracking `t`、tracking broadcast `B`、WATCH dirty-CAS `d`、NO-EVICT `e`、NO-TOUCH `T`，可组合为 `xtB`、`PtB`、`bdeT` 等；`d` 仅在 INFO/LIST 冷路径比较 WATCH 保存版本与当前 Engine 版本后派生，不增加写命令热路径状态，`UNWATCH/EXEC/DISCARD/RESET` 清理观察集后随即消失；事务进入/退出、tracking、MONITOR、阻塞进入/解除及订阅状态变化都会同步注册表，BCAST/NO-EVICT/NO-TOUCH 直接从活跃连接事务读取；当前不支持 TYPE+ID 组合；复制链路客户端尚未接入连接注册表，因此 `MASTER/REPLICA/SLAVE` 类型只提供 standalone 空结果兼容面，`closing` 等尚不存在的状态不会伪造对应 flags
- `CLIENT KILL` 当前支持 legacy `ip:port` 单目标地址形式，以及现代 `ID/ADDR/LADDR/USER/TYPE/MAXAGE/SKIPME` 组合过滤；`LADDR` 使用 accept 时记录的服务端本地地址，`USER` 使用连接认证后同步到注册表的 ACL 用户名，`TYPE` 复用实时 Pub/Sub 分类，`MAXAGE` 使用 accept 时间和当前事件时间的整秒差，均可参与多目标交集过滤
...
- 观察版本失配期间，`CLIENT INFO/LIST` 的 `flags` 包含 dirty-CAS 标志 `d`
- `CLIENT INFO/LIST` 的 Redis 7.4+ `watch` 字段返回当前去重观察 key 数量
