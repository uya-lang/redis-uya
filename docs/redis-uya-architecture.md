...
- `ACL CAT/GENPASS/HELP/WHOAMI` 的无状态兼容面仍可由 `command/executor.uya` 执行；真实连接上的 ACL 状态与门禁由 `connection.uya` 管理。默认用户和每个 named user 使用固定容量模型：最多 8 个 SHA-256 口令摘要和 8 个 selector；每个 selector 独立保存 command/category allow/deny、带读写权限位的 key pattern 与 channel pattern。方向性 key pattern 解析 `~/%R~/%W~/%RW~`，相同 glob 合并权限位，GETUSER/LIST/SAVE 规范化为 `%R~/%W~/~`；固定 key range 复用 `COMMAND GETKEYSANDFLAGS` 的 `access/update/insert/delete` operation 标签，动态 key spec 对 source/destination 和 Streams 容器键补充逐 ordinal 方向。named user 槽位创建、删除和全表清理统一采用 `off/resetpass/resetkeys/resetchannels/-@all` 安全状态，named/default `reset` 通过统一安全状态清除口令、规则、pattern 与 selector；默认用户另有独立 enabled/nopass 状态。服务端 accept 在默认用户 `on nopass` 时把连接初始化为既有默认会话，后续 `off` 不撤销该会话，但新连接进入认证门禁且禁用用户 AUTH 失败。基础用户规则与各 selector 做 OR，但 selector 内要求命令、全部 key 和全部 channel 同时满足，不能跨 selector 拼接权限。普通命令、事务排队、EXEC 重放、脚本内部调用和 DRYRUN 都收敛到统一权限结果函数，并按 command/key/channel 原因记录 ACL LOG 和返回 `NOPERM`。完全开放用户仍保留零 selector 扫描的快速路径
