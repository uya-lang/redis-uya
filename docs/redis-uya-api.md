...
- `CLIENT INFO/LIST` 的 `addr/laddr/fd` 来自 server 接受连接时写入的连接事务和注册表；`fd` 是服务端当前持有的真实 socket 文件描述符；`age/idle` 使用 accept 时刻和最近一次网络读入时刻，通过当前事件循环缓存时间计算整秒差值，不额外读取系统时钟；`cmd` 保存当前正在处理或最近成功解析的命令，输出规范化小写名，`CLIENT/CONFIG/ACL` 等容器命令输出 `parent|subcommand`，从未处理命令时输出 `NULL`，阻塞连接保留阻塞命令名；`redir` 直接读取连接级 `tracking_redirect_id`，未配置或 `TRACKING OFF` 时为 `-1`；`multi` 在事务外为 `-1`，进入 `MULTI` 后输出当前已排队命令数，`EXEC/DISCARD` 后恢复 `-1`；`sub/psub/ssub` 在信息行序列化时按连接 fd 扫描现有固定容量订阅注册表，分别返回普通频道、pattern 和 shard 订阅数，内部聚合订阅计数继续用于回复数量、订阅态限制与客户端类型判断；`qbuf` 返回 server 当前保留的完整未消费输入字节数，当前命令执行时包含本轮完整已读缓冲，`qbuf-free` 返回固定 8192 字节输入缓冲的剩余容量；这两个字段反映 redis-uya 固定缓冲真值，不等同于 Redis SDS 动态分配容量，半包解析时 redis-uya 会保留完整 RESP 前缀；`obl` 返回当前异步发送仍未写入 socket 的逻辑响应字节数，包含零拷贝 GET 尚未发送的对象 body，完整直写时保持 `0`；redis-uya 没有 Redis 式动态回复链表，因此 `oll=0`、`omem=0`，每连接固定 81920 字节输出区和数据库对象本体不计入动态回复内存；列表序列化通过活跃注册表中的事务地址读取 `cmd/redir/multi/qbuf/qbuf-free/obl`，不复制输入输出缓冲或在普通命令路径扫描完整注册表；`db=0` 由 redis-uya 当前固定单数据库配置和 `SELECT` 仅接受 DB 0 的约束保证；`user` 默认返回 `default`，`AUTH` 与 `HELLO AUTH` 的成功或失败状态会立即同步注册表，`RESET` 恢复默认用户；`flags` 按 Redis 顺序暴露当前已支持状态：普通 `N`、MONITOR `O`、Pub/Sub `P`、事务 `x`、阻塞 `b`、tracking `t`，可组合为 `xt`、`Pt`、`bt` 等；事务进入/退出、tracking、MONITOR、阻塞进入/解除及订阅状态变化都会同步注册表；当前不支持 TYPE+ID 组合，也未补齐 Redis 的 `argv-mem/multi-mem/rbs/rbp/tot-mem/events` 等完整统计字段；复制链路客户端尚未接入连接注册表，因此 `MASTER/REPLICA/SLAVE` 类型只提供 standalone 空结果兼容面，关闭中等尚不存在的状态不会伪造对应 flags
