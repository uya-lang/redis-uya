...
- `v0.9.3`：收口 Streams、Functions/Script、ACL 与运维诊断第一批，再推进持久化/复制边界深化；`XADD/XDEL/XGROUP CREATE/XGROUP DESTROY/XGROUP HELP/XINFO HELP/XINFO GROUPS/XINFO CONSUMERS/XINFO STREAM/XLEN/XRANGE/XREVRANGE/XREAD` 已以基础 stream partial 形态落地，`XTRIM` 已以 `MAXLEN` 基础裁剪 partial 形态落地。
...
- Streams partial：`XADD`、`XDEL`、`XGROUP CREATE`、`XGROUP DESTROY`、`XGROUP HELP`、`XINFO HELP`、`XINFO GROUPS`、`XINFO CONSUMERS`、`XINFO STREAM`、`XLEN`、`XRANGE`、`XREVRANGE`、`XREAD`、`XTRIM` 当前可用；当前只支持基础追加、精确 ID 删除、XGROUP/XINFO 帮助兼容面、`XGROUP CREATE` key/type 校验与明确未支持错误、`XGROUP DESTROY` 空状态返回 `0`、基础 stream 元数据、空 consumer group 列表、无 group 时的 `XINFO CONSUMERS` `NOGROUP` 错误、长度、范围读取、非阻塞读取和 `XTRIM MAXLEN [=|~] count` 头部裁剪，尚不支持 `XADD` trim / `NOMKSTREAM` 等选项、`XINFO STREAM FULL`、真实 consumer group 状态和 Redis 原生 radix-tree/listpack 编码。项目内 RDB 与 AOF rewrite 会保存显式 stream ID；普通 AOF append 仍记录原始请求，因此 `XADD *` 回放会重新生成 ID，只承诺恢复条目内容与顺序
