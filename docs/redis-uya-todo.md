...
- [ ] 运维与诊断第一批：`CONFIG` 余量（`latency-tracking` 运行时开关与 `slowlog-log-slower-than` 慢日志门限已补）、`CLIENT` 余量（`REPLY` 命令回复抑制 partial、`UNBLOCK` 阻塞 pop 等待解除 partial、`PAUSE WRITE` 读写区分、`TRACKING` 连接级 flags/redirect/`BCAST PREFIX` 状态与 `CACHING` / `NO-EVICT` / `NO-TOUCH` 连接级兼容标志已补）、`MEMORY` 观测（`MALLOC-STATS` runtime-approx 与 `PURGE` no-op 兼容面已补）、`SLOWLOG`、`LATENCY`、`MONITOR`、`MODULE HELP/LIST` 空模块兼容面已补，`DEBUG` 已按单机安全 profile 收口为 `standalone-error`，后续继续补配置、客户端与观测真值
