...
- ACL 用户名按字节区分大小写，只有精确小写 `default` 指向内建默认用户；例如 `DEFAULT` 可作为独立 named user，并在 SETUSER/GETUSER/AUTH/DRYRUN/WHOAMI/LIST/SAVE/LOAD/DELUSER、ACL LOG 与连接关闭链中保持原始大小写
