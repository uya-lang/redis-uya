...
- `CLIENT INFO`：返回当前连接的客户端信息行，包含真实 `id/addr/laddr/fd/age/idle/db/user/flags` 以及 `name/resp/multi/sub/lib-name/lib-ver`
- `CLIENT LIST [TYPE NORMAL|PUBSUB]` / `CLIENT LIST ID id ...`：无过滤时返回全部活跃连接的信息行快照；`TYPE NORMAL/PUBSUB` 按实时订阅计数分类，`ID` 接受一个或多个客户端 ID 并按请求顺序输出匹配项，重复 ID 会重复输出，缺失 ID 被跳过；当前 TYPE 与 ID 模式互斥，每行包含真实 `id/addr/laddr/fd/age/idle/db/user/flags` 以及 `name/resp/multi/sub/lib-name/lib-ver`
...
- `CLIENT INFO/LIST` 的 `addr/laddr/fd` 来自 server 接受连接时写入的连接事务和注册表；`fd` 是服务端当前持有的真实 socket 文件描述符；`age/idle` 使用 accept 时刻和最近一次网络读入时刻，通过当前事件循环缓存时间计算整秒差值，不额外读取系统时钟；`db=0` 由 redis-uya 当前固定单数据库配置和 `SELECT` 仅接受 DB 0 的约束保证；`user` 默认返回 `default`，`AUTH` 与 `HELLO AUTH` 的成功或失败状态会立即同步注册表，`RESET` 恢复默认用户；`flags` 按 Redis 顺序暴露当前已支持状态：普通 `N`、MONITOR `O`、Pub/Sub `P`、事务 `x`、阻塞 `b`、tracking `t`，可组合为 `xt`、`Pt`、`bt` 等；事务进入/退出、tracking、MONITOR、阻塞进入/解除及订阅状态变化都会同步注册表；当前不支持 `MASTER/REPLICA` 类型或 TYPE+ID 组合，也未补齐 Redis 的 `cmd` 等完整统计字段，复制链路、关闭中等尚不存在的状态不会伪造对应 flags
