...
- [ ] 运维与诊断第一批：`CONFIG` 余量、`CLIENT` 余量（`CACHING` / `NO-EVICT` / `NO-TOUCH` 连接级兼容标志已补）、`MEMORY` 观测（`MALLOC-STATS` runtime-approx 与 `PURGE` no-op 兼容面已补）、`SLOWLOG`、`LATENCY`、`MONITOR`、`MODULE HELP/LIST` 空模块兼容面已补，后续继续补配置、客户端与观测真值
