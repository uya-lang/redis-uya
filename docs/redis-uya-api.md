...
- `ACL LOG [count]` 当前返回命令权限、key pattern 权限、channel pattern 权限与认证拒绝的进程内 ring 日志，覆盖 `ACL DRYRUN`、真实命令、事务队列前置拒绝、脚本内部命令拒绝产生的 `NOPERM`，以及 `AUTH` / `HELLO ... AUTH` 产生的 `WRONGPASS`。命令、key 和 channel 拒绝分别使用 `reason=command/key/channel`，object 分别记录规范化命令名或保持原字节的实际 key/channel；认证条目使用 `reason=auth`、`context=toplevel`、`object=AUTH` 并保留尝试的用户名。同 reason/context/object/username 的事件会在最近 10 条、60 秒窗口内聚合，保留原 `entry-id/timestamp-created` 并更新 `timestamp-last-updated/count/client-info`。字段包含 `reason/context/object/username/age-seconds/client-info/entry-id/timestamp-created/timestamp-last-updated/count`，时间戳使用 Unix 毫秒，`age-seconds` 以最多三位小数保留毫秒精度，其中 `client-info` 会记录最近一次拒绝的真实连接 `id/addr/laddr`；`ACL LOG RESET` 会清空该日志
