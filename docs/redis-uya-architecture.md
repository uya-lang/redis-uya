...
- `ACL CAT/GENPASS/GETUSER/HELP/LIST/LOG/USERS/WHOAMI` 由 `command/executor.uya` 执行，当前只提供 ACL 分类、口令生成、默认用户详情、ACL 控制面帮助、默认用户 config 格式、空 ACL 日志、默认用户列表、默认用户查询与 `COMMAND*` 可见面，ACL 用户存储、命令权限、key pattern 权限、真实 ACL 日志和 ACL 文件加载保存后续再补
