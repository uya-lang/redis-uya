...
def run_smoke() -> None:
    ...
    try:
        with connect_with_retry(port, time.monotonic() + 5.0) as sock:
            ...
            stream_info = send_command(sock, b"COMMAND", b"INFO", b"XACK", b"XADD", b"XDEL", b"XGROUP", b"XINFO", b"XLEN", b"XRANGE", b"XREVRANGE", b"XREAD", b"XTRIM")
            if (
                not isinstance(stream_info, list)
                or len(stream_info) != 10
                ...
                or stream_info[6][0] != b"xrange"
                or not isinstance(stream_info[7], list)
                or stream_info[7][0] != b"xrevrange"
                or not isinstance(stream_info[8], list)
                or stream_info[8][0] != b"xread"
                or not isinstance(stream_info[9], list)
                or stream_info[9][0] != b"xtrim"
            ):
