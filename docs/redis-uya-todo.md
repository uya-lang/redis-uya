...
- [x] Hash 第二批视图：`HKEYS`、`HVALS`、`HGETALL`
- [x] Hash 第三批扫描：`HSCAN`
...
- [x] Server / Connection 缺口第一批：已补 `MEMORY HELP/STATS/USAGE/DOCTOR`（runtime-approx partial）、`SLOWLOG HELP/LEN/GET/RESET`（in-process ring partial）、`LATENCY HELP/LATEST/HISTORY/RESET/DOCTOR/HISTOGRAM/GRAPH`（empty-event compatibility partial）与 `MONITOR`（流式命令观测 partial）
...
- [ ] Streams 第二批：`XTRIM` 已补 `MAXLEN [=|~] count` 基础裁剪 partial，`XDEL` 已补精确 ID 删除 partial，`XGROUP HELP` / `XINFO HELP` 已补帮助兼容面，`XGROUP CREATE` 已补 key/type 校验与明确未支持错误 partial，`XGROUP DESTROY` 已补 empty-state 返回 `0` partial，`XINFO STREAM` 已补 key-only 基础元数据 partial，`XINFO GROUPS` 已补 empty-state 空数组 partial，`XINFO CONSUMERS` 已补无 group 时的 `NOGROUP` 错误面 partial；`XGROUP SETID`、`XINFO STREAM FULL`、`XACK`、`XPENDING`、`XCLAIM` 仍待补
