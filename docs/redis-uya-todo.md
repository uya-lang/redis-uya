...
- [ ] Functions / Script 收口：只读脚本第一批已补 `EVAL_RO` / `EVALSHA_RO` single-call subset partial，Functions 第一批已补 `FUNCTION HELP/LIST/STATS/FLUSH/DELETE/KILL` 与 `FCALL/FCALL_RO` 空库兼容面，后续继续补脚本缓存、错误中止、传播边界和 function library 最小面
