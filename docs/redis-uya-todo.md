...
- 2026-07-23 当前 `HEAD`：`make test`、`make test-integration`、`make test-redis-cli` 与 release benchmark 绝对吞吐、归一化吞吐和 p99 guard 通过；小对象 arena 优化后的同条件 20K 矩阵为 Redis 的 `1.08x/1.29x/1.00x/1.34x/1.09x`，严格 1.10x 全场景超越仍待继续优化。
