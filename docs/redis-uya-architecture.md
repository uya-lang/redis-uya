...
- `FUNCTION HELP/LIST/STATS/FLUSH` 由 `command/executor.uya` 执行，当前只提供 Functions 控制面的帮助、空库列表、空库统计、no-op flush 兼容面和 `COMMAND*` 可见面，function library 存储与 `FCALL*` 执行后续再补
