...
class RedisPySubsetClient:
    ...
    def lmovem(
        ...
        parts = [b"LMOVEM", source.encode(), destination.encode(), wherefrom.encode(), whereto.encode()]
...
def run_smoke() -> None:
    ...
    try:
        ...
        try:
            ...
            try:
                client._request(b"MODULE")
                raise AssertionError("expected MODULE without subcommand to fail")
            except RespError as exc:
                if str(exc) != "ERR wrong number of arguments for 'module' command":
                    raise AssertionError(f"unexpected MODULE arity error: {exc}") from exc
            try:
                client._request(b"SLOWLOG")
                raise AssertionError("expected SLOWLOG without subcommand to fail")
            except RespError as exc:
                if str(exc) != "ERR wrong number of arguments for 'slowlog' command":
                    raise AssertionError(f"unexpected SLOWLOG arity error: {exc}") from exc
            ...
            assert client.rpush("mvsrc", "a", "b", "c", "d") == 4
            assert client.rpush("mvdst", "x") == 1
            assert client.lmovem("mvsrc", "mvdst", "LEFT", "RIGHT", "COUNT", 2, "BULK") == [b"a", b"b"]
            assert client.lrange("mvsrc", 0, -1) == [b"c", b"d"]
            assert client.lrange("mvdst", 0, -1) == [b"x", b"a", b"b"]
            assert client.lmovem("mvsrc", "mvdst", "LEFT", "RIGHT", "EXACTLY", 3, "BULK") is None
            assert client.lmovem("mvsrc", "mvdst", "RIGHT", "LEFT") == [b"d"]
