...
- ACL 第一批 partial：`ACL CAT`、`ACL DELUSER`、`ACL DRYRUN`、`ACL GENPASS`、`ACL GETUSER`、`ACL HELP`、`ACL LIST`、`ACL LOAD`、`ACL LOG`、`ACL SAVE`、`ACL SETUSER`、`ACL USERS`、`ACL WHOAMI` 当前可用，覆盖 default/named user 生命周期、口令认证、命令/分类 deny、key/channel pattern、真实连接与脚本/事务权限拒绝、审计日志和 `COMMAND*` 可见面。`aclfile` 已接入配置文件、`CONFIG GET/SET/REWRITE` 和第七个可选业务启动参数；`ACL SAVE` 以 mode 0600 临时文件完整写入后原子替换，`ACL LOAD` 对唯一 default/named user、64 KiB 上限和 modifier 子集做校验，失败会恢复全部 ACL 状态，启动指定文件时加载失败会拒绝启动。当前 ACL 文件按项目单口令模型保存 `>plaintext` token，依赖 0600 权限保护；`ACL LIST/GETUSER` 仍只回显哈希标记。暂不支持 Redis `#hash` 口令认证、多个口令、selector 权限或动态 key spec / movablekeys 完整解析
