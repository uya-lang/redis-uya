...
- ACL command modifier 先由命令目录规范化 `parent|child` 名称；ACL 请求执行前按目录把容器参数映射到真实 child，通用 `|` parent 提取同时支持父规则继承，因而基础规则、selector、DRYRUN 与 ACL 文件 round-trip 共用子命令授权语义
