...
> 版本: v0.9.0-planning
> 日期: 2026-04-30
> 当前开发工具链: Uya `v0.9.5` 发布线同步副本（来源：`../uya`；注意当前二进制自报版本仍为 `v0.9.4`）
...
- 当前设计以工程内置 `./uya` 工具链为准；其能力边界以上游 `../uya` 当前 `v0.9.5` 发布线同步结果为准。
- 状态记录：
  - `../uya` 的 `v0.9.5` tag 指向 `3d31cf85`
  - 当前参考的 `../uya` HEAD 为 `1d99002d`
  - 当前编译器二进制仍打印 `v0.9.4`，原因是上游版本串未同步更新，不代表 `redis-uya` 仍停留在 `v0.9.4` 工具链基线
