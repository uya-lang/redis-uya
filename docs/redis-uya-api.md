...
- `CLIENT INFO`：返回当前连接的客户端信息行，包含真实 `id/addr/laddr/fd` 以及 `name/resp/multi/sub/lib-name/lib-ver`
- `CLIENT LIST [TYPE NORMAL|PUBSUB]` / `CLIENT LIST ID id ...`：无过滤时返回全部活跃连接的信息行快照；`TYPE NORMAL/PUBSUB` 按实时订阅计数分类，`ID` 接受一个或多个客户端 ID 并按请求顺序输出匹配项，重复 ID 会重复输出，缺失 ID 被跳过；当前 TYPE 与 ID 模式互斥，每行包含真实 `id/addr/laddr/fd` 以及 `name/resp/multi/sub/lib-name/lib-ver`
...
- `CLIENT INFO/LIST` 的 `addr/laddr/fd` 来自 server 接受连接时写入的连接事务和注册表；`fd` 是服务端当前持有的真实 socket 文件描述符，订阅、模式订阅和分片订阅状态变更后会同步注册表，因此 `TYPE NORMAL/PUBSUB` 与 `sub` 字段保持实时；当前不支持 `MASTER/REPLICA` 类型或 TYPE+ID 组合，也未补齐 Redis 的 `age/idle/flags/db/cmd` 等完整统计字段
