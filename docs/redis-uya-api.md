...
CLIENT LIST ID id ...
CLIENT KILL ip:port
CLIENT KILL [ID id] [ADDR ip:port] [USER username] [SKIPME yes|no]
...
- `CLIENT KILL [ID id] [ADDR ip:port] [USER username] [SKIPME yes|no]`：`ID` 与 `ADDR` 定位单个连接，`USER` 按 ACL 用户名匹配一个或多个活跃连接，多个不同类型 selector 取交集；重复 selector 合法，已知用户无连接、目标缺失或条件不相交返回 `0`，未知用户返回 `ERR No such user '<username>'`；默认 `SKIPME yes` 不关闭当前连接，显式 `SKIPME no` 可把当前连接计入关闭结果，返回实际匹配数量
...
- `CLIENT KILL` 当前支持 legacy `ip:port` 单目标地址形式，以及现代 `ID/ADDR/USER/SKIPME` 组合过滤；`USER` 使用连接认证后同步到注册表的 ACL 用户名并支持多目标关闭，当前仍不支持 `LADDR/TYPE/MAXAGE` filters
