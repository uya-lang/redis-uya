...
- [x] ZSet 第三批范围删除、扫描与读/写路径补齐：`ZREMRANGEBYLEX`、`ZREMRANGEBYRANK`、`ZREMRANGEBYSCORE`、`ZRANGESTORE REV/BYSCORE/LIMIT`、`ZSCAN`、`ZRANDMEMBER`、`ZDIFF`、`ZDIFFSTORE`、`ZINTER`、`ZINTERCARD`、`ZINTERSTORE`、`ZUNION`、`ZUNIONSTORE`，其中多 key 聚合命令支持整数 `WEIGHTS` 与 `AGGREGATE SUM|MIN|MAX`
