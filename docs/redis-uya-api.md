...
- `CLIENT INFO`：返回当前连接的客户端信息行，包含真实 `id/addr/laddr/fd/age/idle/db/user/flags/cmd/redir/events` 以及 `name/resp/multi/sub/psub/ssub/qbuf/qbuf-free/argv-mem/multi-mem/rbs/rbp/obl/oll/omem/tot-mem/lib-name/lib-ver`；已支持状态中 tracking BCAST、NO-EVICT、NO-TOUCH 分别追加 `B/e/T` 标志
