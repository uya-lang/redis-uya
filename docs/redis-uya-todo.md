...
- [x] Server / Connection 缺口第一批：已补 `MEMORY HELP/STATS/USAGE/DOCTOR/PURGE`（runtime-approx/no-op partial）、`SLOWLOG HELP/LEN/GET/RESET`（in-process ring partial）、`LATENCY HELP/LATEST/HISTORY/RESET/DOCTOR/HISTOGRAM/GRAPH`（empty-event compatibility partial）与 `MONITOR`（流式命令观测 partial）
...
- [ ] 运维与诊断第一批：`CONFIG` 余量、`CLIENT` 余量、`MEMORY` 观测（`PURGE` no-op 兼容面已补）、`SLOWLOG`、`LATENCY`、`MONITOR`
