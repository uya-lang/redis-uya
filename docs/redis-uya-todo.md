...
- [ ] 运维与诊断第一批：`CONFIG` 余量（`timeout` 已补运行时回读、重写和普通空闲连接关闭，`latency-tracking` 运行时开关、`latency-monitor-threshold` 事件门限、`slowlog-log-slower-than` 慢日志门限和 `slowlog-max-len` 保留长度已补）、`CLIENT` 余量（`INFO/LIST` 已补真实 `addr/laddr/fd/age/idle/cmd/redir/events/io-thread/multi/watch/sub/psub/ssub/qbuf/qbuf-free/argv-mem/multi-mem/rbs/rbp/obl/oll/omem/tot-mem` 观测、单数据库 `db=0`、ACL `user` 与当前支持连接状态 `N/O/P/x/b/t/B/d/e/T flags` 真值、`LIST ID id...` 多值过滤、`LIST TYPE NORMAL|PUBSUB` 实时订阅分类及 `MASTER|REPLICA|SLAVE` standalone 空结果兼容，legacy `KILL ip:port` 单目标及现代 `KILL ID/ADDR/SKIPME` 单目标交集 partial、`REPLY` 命令回复抑制 partial、`UNBLOCK` 阻塞 pop 等待解除 partial、`PAUSE WRITE` 读写区分及非法 timeout 完整错误文本、`TRACKING` 连接级 flags/redirect/`BCAST PREFIX` 状态、WATCH key 计数与 dirty-CAS 冷路径真值、Redis 8.0 单事件循环 `io-thread=0`、`CACHING` / `NO-EVICT` 连接级兼容标志与 `NO-TOUCH` 对象访问 LRU/LFU touch 抑制已补）、`MEMORY` 观测（`MALLOC-STATS` runtime-approx、`PURGE` no-op 兼容面与容器 arity 完整错误文本已补）、`SLOWLOG` 容器 arity 完整错误文本、`LATENCY`（含 top-level 与容器子命令直方图）、`MONITOR`、`MODULE HELP/LIST` 空模块兼容面及容器 arity 完整错误文本已补，`MODULE LOAD/LOADEX/UNLOAD` 与 Redis Array `AR*`、RediSearch `FT.*`、RedisJSON `JSON.*`、Redis Vector Set `V*`、RedisBloom `BF.*` / `CF.*` / `CMS.*` / `TOPK.*` / `TDIGEST.*`、RedisTimeSeries `TS.*` 已按单机安全 profile 收口为 `standalone-error`，`DEBUG` 已按单机安全 profile 收口为 `standalone-error`，`HOTKEYS HELP/GET/RESET/START/STOP` 已补空采样/no-op 诊断兼容面 partial，Sharded Pub/Sub 已补 `SSUBSCRIBE/SUNSUBSCRIBE/SPUBLISH` standalone 连接级 partial 与 `PUBSUB SHARDCHANNELS/SHARDNUMSUB` 真值，后续继续补配置、客户端与观测真值
