...
截至 2026-05-16 审计，项目的正确口径应为：

- `v0.9.0` 的历史收口文档仍然保留，但不能直接代表当前 `HEAD`。
- 当前 `HEAD` 的 `make test` 通过，但 `make test-integration` 与 `make benchmark-v0.8.1` 仍未恢复为全绿。
- 当前主线的第一优先级不是继续扩展目录面，而是修复“控制面、测试、benchmark、版本号、文档”之间的真实性问题。
