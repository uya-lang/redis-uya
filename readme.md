...
当前项目已完成 `v0.9.0` 单机核心命令补齐：按计划补齐 String / Hash / List / Set / ZSet / Key / Server / Security 核心命令批次，收口 `SORT`、`AUTH`、`requirepass`、`SHUTDOWN`。当前主线已进入 `v0.9.1` 第一批，新增官方命令全集矩阵、共享运行时目录和 `COMMAND` / `COMMAND COUNT` / `COMMAND LIST` / `COMMAND INFO` / `COMMAND DOCS` 控制面。
...
- `v0.9.1`：继续推进单机命令全集矩阵、连接/管理面补齐和兼容边界收敛；当前已完成官方命令全集矩阵、`COMMAND*` 第一批、`COMMAND GETKEYS*` 当前命令表支持、全局 `CLIENT LIST`、`CONFIG SET` 运行时子集，以及 `CLIENT KILL/PAUSE/TRACKING` 最小闭环，待续 `COMMAND DOCS` 全量输出、`CONFIG REWRITE`、tracking invalidation、pattern Pub/Sub、Scripting/Stream 命令族与 standalone 错误边界。
...
- 当前同步源：`../uya` 当前发布线（按 `v0.9.5` 工具链能力使用）
- C 宿主工具链：`cc`、`gcc` 或 `clang`
- 目标平台：Linux x86_64 / ARM64

说明：

- `../uya` 已存在 `v0.9.5` tag，标签提交为 `3d31cf85`
- 当前同步时参考的 `../uya` 工作线 HEAD 为 `1d99002d`，即位于 `v0.9.5` tag 之后 2 个提交
- 现阶段 `./uya/bin/uya --version` 仍输出 `v0.9.4`，原因不是同步失败，而是上游 `../uya/src/main.uya` 中的版本串尚未从 `v0.9.4` 改到 `v0.9.5`
- 因此，`redis-uya` 这里记录的“按 `v0.9.5` 工具链能力使用”，指的是以上游 `../uya` 的 tag / 代码线和同步后的标准库能力为准，而不是以编译器当前自报版本串为准
- `uya/` 内置副本需要按 `../uya` 当前发布线持续同步；后续开发默认可以使用当前内置工具链已经支持的 `v0.9.5` 能力
...
- 完整 `CONFIG REWRITE`、`CONFIG SET` 其余字段，以及更完整的 `CLIENT KILL/PAUSE/TRACKING`
