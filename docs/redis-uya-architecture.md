...
- `AUTH` 与 `HELLO ... AUTH` 的 `WRONGPASS` 分支在清理连接认证态并同步注册表后写入 ACL LOG，使用 `reason=auth`、`context=toplevel`、大写 `AUTH` object 和本次尝试的原始用户名；普通 `AUTH` 与 HELLO 内嵌认证共用同一审计入口。ACL LOG 以独立 order 维护最近更新时间顺序，扫描最近 10 条并在 60 秒窗口内按 reason/context/object/username 聚合，保留创建标识和时间、刷新最近客户端信息，满 128 条时淘汰 order 最旧项
