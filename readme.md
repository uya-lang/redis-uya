...
- Hash 命令子集：`HSET`、`HGET`、`HDEL`、`HEXISTS`、`HLEN`、`HMGET`、`HMSET`、`HSETNX`、`HSTRLEN`、`HRANDFIELD`，field TTL 兼容面 `HGETEX/HSETEX/HEXPIRE/HPEXPIRE/HEXPIREAT/HPEXPIREAT/HTTL/HPTTL/HEXPIRETIME/HPEXPIRETIME/HPERSIST`，以及连接级 fieldset 导入 `HIMPORT PREPARE/SET/DISCARD/DISCARDALL`
...
- `v0.9.3`：收口 Streams、Functions/Script、ACL 与运维诊断第一批，再推进持久化/复制边界深化；`XACK/XACKDEL/XADD/XAUTOCLAIM/XCFGSET/XCLAIM/XDEL/XDELEX/XGROUP CREATE/XGROUP CREATECONSUMER/XGROUP DELCONSUMER/XGROUP DESTROY/XGROUP HELP/XGROUP SETID/XIDMPRECORD/XINFO HELP/XINFO GROUPS/XINFO CONSUMERS/XINFO STREAM/XLEN/XNACK/XPENDING/XRANGE/XREVRANGE/XREAD/XREADGROUP/XSETID` 已以基础 stream partial 形态落地，`XTRIM` 已以 `MAXLEN` 基础裁剪 partial 形态落地，`EVAL_RO/EVALSHA_RO` 已以 single-call read-only script subset partial 形态落地，`SCRIPT DEBUG` 已以 no-op 兼容面 partial 形态落地，`SCRIPT KILL` 已以无运行脚本 `NOTBUSY` 错误面 partial 形态落地；Functions 已具备最小 library 存储、`FCALL*` 执行和 Redis 7 格式非空 `FUNCTION DUMP/RESTORE` round-trip partial，`ACL CAT/DELUSER/DRYRUN/GENPASS/GETUSER/HELP/LIST/LOAD/LOG/SAVE/SETUSER/USERS/WHOAMI` 已以 default/named user、命令/分类 deny、key/channel pattern 权限 partial 形态落地，`MODULE HELP/LIST` 已以空模块兼容 partial 形态落地，`CLIENT REPLY/UNBLOCK/CACHING/NO-EVICT/NO-TOUCH` 已以连接级/回复面/阻塞解除 partial 形态落地，`WAITAOF` 已以本地/AOF 确认数组 partial 形态落地。
...
- 当前同步源：`../uya` 的 `1.0` 分支，提交 `f54bd7bf`
- C 宿主工具链：`cc`、`gcc` 或 `clang`
- 目标平台：Linux x86_64 / ARM64

说明：

- `./uya/bin/uya --version` 返回 `v0.9.9`；该文件是 188,776 字节的静态命令启动器，实际 `build` 子命令由 `./uya/bin/cmd/build` 执行
- `./uya/bin/cmd/build` 为 6,924,448 字节的动态链接编译器；同步时必须同时更新启动器和该子命令，不能只复制 `bin/uya`
- 当前同步产物 SHA256 分别为 `8ad1e481e45a65907b4ff02cac48646799d804e90ce794e9445680fc5e6359cf` 和 `883f369402b0f6a2eed5fbad08b5d5d4221108c355a42f54b2657513f6a8332d`
- 本次同步、规范回退、项目兼容调整和验证结果见 [Uya 1.0 同步报告（2026-07-22，2026-07-23 修订）](docs/redis-uya-uya-sync-2026-07-22.md)
- `uya/` 内置副本需要按 `../uya` 的目标分支持续同步；后续开发默认只使用已经由 redis-uya 构建与测试证明可用的 `v0.9.9` 能力
...
- ACL 第一批 partial：`ACL CAT`、`ACL DELUSER`、`ACL DRYRUN`、`ACL GENPASS`、`ACL GETUSER`、`ACL HELP`、`ACL LIST`、`ACL LOAD`、`ACL LOG`、`ACL SAVE`、`ACL SETUSER`、`ACL USERS`、`ACL WHOAMI` 当前可用，用于暴露 ACL 分类、默认用户不可删除错误面、默认用户 dry-run 命令检查、口令生成、默认用户详情、ACL 控制面帮助、默认用户 config 格式、`requirepass` 哈希标记回显、ACL 文件未配置错误面、默认用户列表、当前默认用户、named user 进程内元数据、口令写入与认证、默认用户和 named user 命令级 `+cmd/-cmd` 与分类级 `+@category/-@category` 允许/拒绝路径、`resetcommands` 命令规则恢复、`allkeys/resetkeys/~pattern` 固定 key range 权限 partial、`allchannels/resetchannels/&pattern` Pub/Sub channel 权限 partial、`clearselectors/resetselectors` selector 兼容 no-op、ACL 拒绝日志和 `COMMAND*` 可见面；`ACL CAT category` 当前返回可见命令目录中的匹配命令，`ACL DELUSER` 当前不会删除默认用户，`ACL DRYRUN default ...` 当前会校验命令存在性、arity、默认用户命令 deny list、分类 deny list、key pattern 与 channel pattern，已创建 named user 的 `DRYRUN` 会检查用户存在性、命令存在性、arity 与该 named user 自身规则，但不套用 default 用户 deny list，`ACL SETUSER default -get` 或 `-@string` 会让 default 用户连接后续 `GET` 和 `ACL DRYRUN default GET ...` 返回命令 `NOPERM`，`ACL SETUSER default resetkeys` / `~safe*` 会限制固定 key range 命令，`ACL SETUSER default resetchannels` / `&safe*` 会限制 `PUBLISH/SUBSCRIBE/PSUBSCRIBE/SSUBSCRIBE` 等 Pub/Sub channel 参数，`allkeys/~*` 与 `allchannels/&*` 分别恢复全 key 和全 channel 访问，`ACL SETUSER <name> on >password` 可创建 named user 并用于 `AUTH username password` / `HELLO ... AUTH username password`，`ACL SETUSER <name> -get`、`-@string`、`resetkeys/~pattern` 或 `resetchannels/&pattern` 会影响该 named user 的 `DRYRUN` 与已认证连接权限，`resetcommands` 恢复该 named user 命令规则，成功后 `ACL WHOAMI` 返回该用户名，`ACL GENPASS [bits]` 当前返回 Redis 兼容长度的十六进制口令，`ACL LIST` / `ACL GETUSER` 会反映当前命令、分类、key/channel 规则与 `requirepass` 只读哈希标记，`ACL LOAD` / `ACL SAVE` 当前按未配置 ACL 文件的 Redis 兼容错误返回，`ACL LOG [count]` 当前返回进程内命令/key/channel 权限拒绝日志，含基础审计字段和拒绝发生时的真实 client id/addr/laddr 并支持 `ACL LOG RESET` 清空；暂不支持完整 Redis 分类授权模型、selector 权限或 ACL 文件加载保存，安全基线仍由 `requirepass` / `AUTH` 提供
...
- Hash 第三批扫描：`HSCAN`
- Hash legacy alias：`HMSET`
- Hash field TTL 兼容面：`HGETEX`、`HSETEX`、`HEXPIRE`、`HPEXPIRE`、`HEXPIREAT`、`HPEXPIREAT`、`HTTL`、`HPTTL`、`HEXPIRETIME`、`HPEXPIRETIME`、`HPERSIST`
...
- Set 第五批基数读取：`SINTERCARD`、`SDIFFCARD`、`SUNIONCARD` 支持动态 `numkeys` 与 `LIMIT`；`SUNIONCARD APPROX` 当前返回精确基数，作为零误差兼容回退
