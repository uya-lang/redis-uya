...
- [ ] Streams 第二批：`XTRIM` 已补 `MAXLEN [=|~] count` 基础裁剪 partial，`XDEL` 已补精确 ID 删除 partial，`XGROUP HELP` / `XINFO HELP` 已补帮助兼容面；`XGROUP CREATE/DESTROY/SETID`、`XINFO STREAM/GROUPS/CONSUMERS`、`XACK`、`XPENDING`、`XCLAIM` 仍待补
