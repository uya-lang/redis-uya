...
- `ACL CAT/GENPASS/HELP/WHOAMI` 的无状态兼容面仍可由 `command/executor.uya` 执行；真实连接上的 `ACL SETUSER/DRYRUN/LIST/GETUSER/USERS/DELUSER/LOG/LOAD/SAVE` 由 `connection.uya` 接管默认用户命令/分类 deny list、key/channel pattern、named user 元数据、口令、认证和拒绝日志，以便在执行前统一返回 `NOPERM`。`aclfile` 进入 `ServerConfig`、运行时 CONFIG 与 rewrite；`ACL SAVE` 先规范化当前支持子集，再以 mode `0600` 排他重建 `.tmp`、完整写入并关闭后原子 rename；`ACL LOAD` 读取不超过 64 KiB 的文件，先快照全部 ACL 表和候选配置，解析成功才提交，任何文件/语法/容量错误都恢复快照。加载要求唯一 `default` 和唯一 named user 声明，启动参数指定 ACL 文件时由 `server_set_aclfile_and_load()` 在事件循环前执行，失败则拒绝启动。当前文件格式保存单个 `>plaintext` 口令以保持项目现有认证模型可重启，不支持 Redis `#hash` 认证、多个口令或 selector；`ACL LIST/GETUSER` 仍只暴露哈希标记。默认用户与 named user 分别维护活跃命令/分类拒绝规则计数，规则表为空时跳过固定 deny 表或生成命令目录扫描；当前支持 `+cmd/-cmd`、`+@category/-@category`、`resetcommands`、`allkeys/resetkeys/~pattern`、`allchannels/resetchannels/&pattern`、named user 生命周期、`>password/<password/resetpass/nopass`、`AUTH` / `HELLO AUTH`、事务/脚本权限检查和 ACL LOG 基础审计。selector 权限和 Redis 动态 key spec / movablekeys 完整解析后续再补
