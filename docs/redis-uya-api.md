...
- `ACL GETUSER default` 返回当前默认用户详情，并在 `commands`、`keys` 与 `channels` 字段中反映当前命令/分类 deny list、key pattern 和 channel pattern 规则；启用 `requirepass` 或 ACL 哈希口令时 `flags` 不再包含 `nopass`，`passwords` 按插入顺序返回不带 `#` 的 64 位小写十六进制 SHA-256 数组而不暴露明文；已创建 named user 返回 `flags/passwords/commands/keys/channels/selectors` 元数据视图，`>password` 写入的口令会立即转为 SHA-256，`#hash` 可直接写入摘要；未知用户名返回 null
...
- `ACL LOAD` 从 `aclfile` 指定文件原子加载默认用户、named user、每用户最多 8 个并存口令、命令/分类 deny list、key pattern 和 channel pattern；重复摘要去重并保留首次插入顺序，文件不可读、超过 64 KiB、口令容量超限、缺少或重复 `default`、重复 named user、包含未支持 modifier 时返回错误并完整恢复加载前状态
...
- `ACL SETUSER default [attribute ...]` 当前支持 `on`、`nopass`、`resetpass`、`>password`、`<password`、`#hash`、`!hash`、`&*`、`+@all`、`clearselectors`、`resetselectors` 兼容面，也支持命令级 `-cmd` / `+cmd`、分类级 `-@category` / `+@category`、`resetcommands`、`allkeys`、`resetkeys`、`~pattern`、`allchannels`、`resetchannels` 和 `&pattern`；默认用户最多保存 8 个并存 SHA-256 摘要，重复添加幂等，`<password` / `!hash` 只删除命中的单个摘要并稳定保留其余顺序，容量超限返回明确错误。`>password` 会立即转为 SHA-256，`#hash` 只接受 64 位小写十六进制，删除不存在的口令时返回 Redis 兼容错误。default 的 ACL 哈希口令不通过 `CONFIG GET requirepass` 反向暴露；后续 `CONFIG SET requirepass` 会切换回兼容明文配置来源并清除全部旧摘要。例如 `ACL SETUSER default -get` 或 `ACL SETUSER default -@string` 会让 default 用户连接的后续 `GET` 与 `ACL DRYRUN default GET ...` 返回命令 `NOPERM`
- `ACL SETUSER <named-user> [attribute ...]` 当前支持进程内 named user 元数据创建与更新，覆盖 `on`、`off`、`nopass`、`resetpass`、`>password`、`<password`、`#hash`、`!hash`、`~*`、`&*`、`+@all`、`allkeys`、`resetkeys`、`~pattern`、`allchannels`、`resetchannels`、`&pattern`、`clearselectors`、`resetselectors`、`-cmd/+cmd` 和 `-@category/+@category`；每用户最多保存 8 个并存 SHA-256 摘要，重复添加幂等，`AUTH username password` / `HELLO ... AUTH username password` 对候选口令计算摘要后依次比较，`nopass/resetpass` 清空全部摘要，`<password` / `!hash` 只删除指定摘要；当前仍不支持 selector 真实授权
...
- 当前 ACL 文件规范输出每个用户按插入顺序排列的 SHA-256 `#hash`，不会保存明文；加载器同时接受多个 `#hash` 和旧版 `>plaintext` token，旧 token 在加载时立即转成摘要，下一次 `ACL SAVE` 会迁移为 `#hash`。每用户上限为 8 个唯一摘要，重复 token 幂等，超限会让整次 LOAD 回滚。当前仍不支持 selector 或不可由空白分隔文本无损表示的用户名/pattern
