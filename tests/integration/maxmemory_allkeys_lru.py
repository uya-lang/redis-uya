...
def run_smoke() -> None:
    ...
    try:
        with connect_with_retry(port, time.monotonic() + 5.0) as sock:
            ...
            if not isinstance(config, list) or array_pairs_to_dict(config).get("maxmemory-policy") != "allkeys-lru":
                raise AssertionError(f"unexpected CONFIG GET maxmemory-policy: {config!r}")

            info = send_command(sock, b"INFO", b"memory")
            if not isinstance(info, bytes) or b"maxmemory_policy:allkeys-lru" not in info:
                raise AssertionError(f"unexpected INFO memory: {info!r}")

            old_value = b"a" * 900
            hot_value = b"b" * 900
            new_value = b"c" * 900
            if send_command(sock, b"SET", b"old", old_value) != "OK":
                raise AssertionError("SET old failed")
            time.sleep(0.02)
            ...
            if send_command(sock, b"GET", b"old") is not None:
                raise AssertionError("old key should be evicted by allkeys-lru")
            if send_command(sock, b"GET", b"hot") != hot_value:
                raise AssertionError("hot key should survive allkeys-lru")
...
def main() -> int:
    try:
        ...
        print(f"[FAIL] integration/maxmemory_allkeys_lru: {exc}", file=sys.stderr)
        return 1
    print("[PASS] integration/maxmemory_allkeys_lru")
    return 0
