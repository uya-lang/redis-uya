...
这个调整的核心原因是：当前最大风险不是“能不能继续堆集群能力”，而是单机命令面、验证链条、控制面真实性、安全运维面和写路径性能仍未达到封版要求。过早扩大集群状态机会放大测试和性能债务。
...
- 虽然已经有官方命令全集目录，但当前 `COMMAND*` 仍可能把 `deferred` 命令包装成“已知命令”，控制面真实性不足。
- Streams、Lua、Functions、ACL、TLS、CLIENT/CONFIG 管理面仍需补齐。
- Bitmap、Bitfield、HyperLogLog、Geo 等 Redis Open Source 单机核心能力仍未形成完整实现。
- JSON、Search、Time Series、概率结构和 Vector 等模块能力也未形成完整实现，但它们不应继续与单机核心完成度混算。
...
每个官方命令名都必须进入 `redis-uya-command-scope.md` 定义的兼容矩阵。但 `v1.0.0` 的封版门槛必须先以 Redis Open Source 单机核心为主，模块命令继续追踪但不与当前单机完成度混算。非模式相关命令在 `v1.0.0` 前必须达到完整 standalone 语义；Cluster/Sentinel 等模式相关命令在 `v1.0.0` 前至少达到 standalone 兼容错误或明确行为。
...
- 运行时 `COMMAND*` 输出必须与真实执行面一致，不能把 `deferred` 命令伪装成可执行支持。
- 如果目录继续追踪模块命令，也必须在统计和文档中与单机封版门槛分开。
...
集中处理性能、稳定性、长时运行、故障恢复、内存泄漏、慢客户端、磁盘损坏矩阵、兼容矩阵和发布文档。此阶段不再扩大功能范围，只允许修缺口和稳定性问题。`v0.9.5` 未达标时继续 `v0.9.6` 等 patch 版本。
...
当前正确策略是先完成单机版，而不是继续扩大集群状态机。已有集群基础可以保留为历史能力和后续参考，但 `redis-uya` 的下一个重大目标应当是：从 `v0.9.0` 开始按 patch 位顺序迭代，经过 `v0.9.1`、`v0.9.2`、`v0.9.3`、`v0.9.4`、`v0.9.5`，必要时继续 `v0.9.6`，直到单机功能、兼容性、性能和稳定性达到 `v1.0.0` 封版标准。
