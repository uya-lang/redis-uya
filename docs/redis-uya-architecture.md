...
- `ACL CAT/GENPASS/HELP/LOAD/SAVE/WHOAMI` 仍由 `command/executor.uya` 执行；`ACL SETUSER/DRYRUN/LIST/GETUSER/USERS/DELUSER/LOG` 在真实连接路径由 `connection.uya` 接管默认用户的进程内命令 deny list、分类 deny list、named user 元数据、`requirepass` 只读回显和拒绝日志，以便在命令执行前统一返回 `NOPERM` 并记录 `ACL LOG`。当前支持默认用户命令级 `+cmd/-cmd`、分类级 `+@category/-@category` 允许/拒绝、`resetcommands` 清空命令与分类拒绝、`resetkeys/resetchannels/clearselectors/resetselectors` 固定默认视图兼容 no-op、named user 进程内创建/列出/详情/删除和 dry-run 用户存在性检查、`ACL LIST` / `ACL GETUSER default` 回显 deny list 与 `requirepass` 哈希标记、事务队列前置拒绝、脚本内部命令拒绝和默认用户命令权限拒绝 ring 日志基础审计字段，`client-info` 记录拒绝发生时的真实连接 id、addr 与 laddr；named user 认证、密码管理写入、完整 Redis 分类授权模型、key pattern 权限、selector 权限和 ACL 文件加载保存后续再补
