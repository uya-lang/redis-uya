...
- [x] Hash 第三批扫描：`HSCAN`
- [x] Hash 字段扩展：`HGETDEL`
- [x] Hash field TTL 查询/清理兼容面：`HGETEX`、`HTTL`、`HPTTL`、`HEXPIRETIME`、`HPEXPIRETIME`、`HPERSIST` 已补无 field TTL 元数据场景的 partial；`HGETEX` 返回 field 值并校验 TTL option，TTL 查询存在 field 返回 `-1`，缺失 field/key 返回 `-2`，真实 field TTL 写入和过期仍待后续补齐
