...
- ACL 第一批 partial：`ACL HELP` 当前可用，用于暴露 ACL 控制面帮助和 `COMMAND*` 可见面；暂不支持 ACL 用户存储、命令权限、key pattern 权限、ACL 日志或 ACL 文件加载保存，安全基线仍由 `requirepass` / `AUTH` 提供
