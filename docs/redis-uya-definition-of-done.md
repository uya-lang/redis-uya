...
| DoD 项 | 证据 |
|--------|------|
| 运维、安全与可观测阶段尚未进入执行期；ACL / TLS / 更完整 `CLIENT/CONFIG/INFO/SLOWLOG/LATENCY/MEMORY/MONITOR` 的证据待进入执行期后补齐，其中 `LATENCY` 当前仅完成空事件兼容子集 | 以 `docs/redis-uya-todo.md` 当前 `v0.9.3` 计划为准 |
