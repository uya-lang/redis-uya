...
INCREX_SATURATE_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" increx excounter byint 9 ubound 10 saturate)"
if [[ "$INCREX_SATURATE_RESULT" != $'10\n6' ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected INCREX saturate 10/6, got '$INCREX_SATURATE_RESULT'" >&2
    exit 1
fi
...
ACL_DYNAMIC_GEORADIUS_UNSAFE_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" acl dryrun dynamic georadius safe:source 0 0 1 km count 1 any store unsafe 2>&1 || true)"
if [[ "$ACL_DYNAMIC_GEORADIUS_UNSAFE_RESULT" != "NOPERM User dynamic has no permissions to access one of the keys used as arguments" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected ACL GEORADIUS unsafe STORE destination denial, got '$ACL_DYNAMIC_GEORADIUS_UNSAFE_RESULT'" >&2
    exit 1
fi

ACL_DYNAMIC_GEORADIUSBYMEMBER_UNSAFE_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" acl dryrun dynamic georadiusbymember safe:source m 1 km storedist unsafe 2>&1 || true)"
if [[ "$ACL_DYNAMIC_GEORADIUSBYMEMBER_UNSAFE_RESULT" != "NOPERM User dynamic has no permissions to access one of the keys used as arguments" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected ACL GEORADIUSBYMEMBER unsafe STOREDIST destination denial, got '$ACL_DYNAMIC_GEORADIUSBYMEMBER_UNSAFE_RESULT'" >&2
    exit 1
fi
...
ACL_RESET_PATTERNS_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" acl setuser default resetkeys resetchannels)"
if [[ "$ACL_RESET_PATTERNS_RESULT" != "OK" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected ACL SETUSER default resetkeys/resetchannels OK, got '$ACL_RESET_PATTERNS_RESULT'" >&2
    exit 1
fi
...
TDIGEST_ADD_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" tdigest.add td 1 2>&1 || true)"
if [[ "$TDIGEST_ADD_RESULT" != "ERR TDIGEST.ADD command not allowed by redis-uya standalone profile" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected TDIGEST.ADD disabled error, got '$TDIGEST_ADD_RESULT'" >&2
    exit 1
fi
...
CLIENT_KILL_TYPE_REPLICA_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" client kill type replica)"
if [[ "$CLIENT_KILL_TYPE_REPLICA_RESULT" != "0" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected standalone CLIENT KILL TYPE replica 0, got '$CLIENT_KILL_TYPE_REPLICA_RESULT'" >&2
    exit 1
fi
...
ZRANGESTORE_BYSCORE_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" zrangestore zrangestorescore zset 2 4 byscore)"
if [[ "$ZRANGESTORE_BYSCORE_RESULT" != "2" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected ZRANGESTORE BYSCORE count 2, got '$ZRANGESTORE_BYSCORE_RESULT'" >&2
    exit 1
fi

ZRANGESTORE_BYSCORE_RANGE_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" zrange zrangestorescore 0 -1)"
if [[ "$ZRANGESTORE_BYSCORE_RANGE_RESULT" != $'b\na' ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected ZRANGESTORE BYSCORE target b/a, got '$ZRANGESTORE_BYSCORE_RANGE_RESULT'" >&2
    exit 1
fi

ZRANGESTORE_BYSCORE_SCORE_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" zscore zrangestorescore a)"
if [[ "$ZRANGESTORE_BYSCORE_SCORE_RESULT" != "4" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected ZRANGESTORE BYSCORE score 4, got '$ZRANGESTORE_BYSCORE_SCORE_RESULT'" >&2
    exit 1
fi

ZRANGESTORE_BYSCORE_REV_LIMIT_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" zrangestore zrangestorescorerev zset 4 2 byscore rev limit 1 1)"
if [[ "$ZRANGESTORE_BYSCORE_REV_LIMIT_RESULT" != "1" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected ZRANGESTORE BYSCORE REV LIMIT count 1, got '$ZRANGESTORE_BYSCORE_REV_LIMIT_RESULT'" >&2
    exit 1
fi

ZRANGESTORE_BYSCORE_REV_LIMIT_RANGE_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" zrange zrangestorescorerev 0 -1)"
if [[ "$ZRANGESTORE_BYSCORE_REV_LIMIT_RANGE_RESULT" != "b" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected ZRANGESTORE BYSCORE REV LIMIT target b, got '$ZRANGESTORE_BYSCORE_REV_LIMIT_RANGE_RESULT'" >&2
    exit 1
fi
...
ZRANGEBYSCORE_LIMIT_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" zrangebyscore zset 2 4 limit 1 1)"
if [[ "$ZRANGEBYSCORE_LIMIT_RESULT" != "a" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected ZRANGEBYSCORE LIMIT a, got '$ZRANGEBYSCORE_LIMIT_RESULT'" >&2
    exit 1
fi

ZREVRANGEBYSCORE_LIMIT_WITHSCORES_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" zrevrangebyscore zset 4 2 withscores limit 1 1)"
if [[ "$ZREVRANGEBYSCORE_LIMIT_WITHSCORES_RESULT" != $'b\n2' ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected ZREVRANGEBYSCORE WITHSCORES LIMIT b/2, got '$ZREVRANGEBYSCORE_LIMIT_WITHSCORES_RESULT'" >&2
    exit 1
fi
...
ZSET_DEL_RESULT="$(redis-cli --raw -h 127.0.0.1 -p "$PORT" del zset zrangestorerev zrangestorescore zrangestorescorerev)"
if [[ "$ZSET_DEL_RESULT" != "4" ]]; then
    echo "[FAIL] integration/redis_cli_smoke: expected zset DEL 4, got '$ZSET_DEL_RESULT'" >&2
    exit 1
fi
