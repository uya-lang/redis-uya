...
- `CLIENT INFO/LIST` 复用同一信息行编码路径；远端 `addr`、本地 `laddr` 与服务端 socket `fd` 由 `server.uya` 接受连接时写入连接事务和注册表，当前连接直接读取事务、列表项读取注册表；当前配置强制单数据库且 `SELECT` 只接受 `0`，共享编码器据此输出 `db=0`；订阅/退订路径同步 `pubsub_count`，`LIST` 可扫描活跃连接、按 NORMAL/PUBSUB 分类或按请求顺序查询多个 ID，连接关闭时由 server 注销
