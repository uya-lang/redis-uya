...
- 控制面当前覆盖 `CLIENT` / `CONFIG` 的兼容子集，`CONFIG SET` 已支持 `port/bind/dir/dbfilename/appendfilename/requirepass/masterauth/replicaof/maxmemory/maxmemory-policy/maxclients/databases/timeout/save/latency-tracking/latency-monitor-threshold/slowlog-log-slower-than/slowlog-max-len` 运行时子集，`CONFIG REWRITE` 已支持把当前有效配置写到 `<appendfilename>.conf`，其中 `timeout` 会驱动 server cron 关闭普通空闲连接；`CLIENT KILL` 已支持 legacy 地址和现代 `ID/ADDR/LADDR/USER/TYPE/SKIPME` 交集过滤，`CLIENT PAUSE WRITE` 可区分读写命令，`CLIENT TRACKING` 可回显 `BCAST PREFIX` 列表；仍没有其余 `CONFIG SET` 热更新、更完整的 rewrite 保真度、tracking invalidation，以及 `MAXAGE` 等 richer client filters
