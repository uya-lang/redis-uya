...
- Scripting 第一批 partial：`EVAL`、`EVALSHA`、`EVAL_RO`、`EVALSHA_RO`、`SCRIPT LOAD/EXISTS/FLUSH/KILL` 当前可用，但只支持单条 `return redis.call(...)` 脚本子集；`*_RO` 会拒绝内部写命令，`SCRIPT KILL` 只覆盖无运行脚本 `NOTBUSY` 错误面；AOF/复制传播的是脚本内部实际执行的命令效果，而不是原始 `EVAL*`
