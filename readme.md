...
- Latency 第一批 partial：`LATENCY HELP`、`LATENCY LATEST`、`LATENCY HISTORY`、`LATENCY RESET`、`LATENCY DOCTOR`、`LATENCY HISTOGRAM`、`LATENCY GRAPH` 当前可用；当前采样 `command` 事件的运行时耗时并保留进程内历史，耗时精度受毫秒级时间源限制，`HISTOGRAM` 仍为空直方图兼容面，尚未实现 Redis 原生 `latency-monitor-threshold` 事件门限
