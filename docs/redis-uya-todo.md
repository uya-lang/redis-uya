...
- [x] 复制角色与状态机
- [x] `PSYNC` / backlog
- [x] `REPLCONF` 握手 partial：常见选项返回 `OK`，ACK/FACK/GETACK 具备连接级 offset 记录与 wire 回复语义
- [x] 全量同步
- [x] 增量同步
- [x] 复制心跳
- [x] 集成测试：主从一致性
...
- [x] AOF 持久化位置前置能力：writer 区分 appended/flushed/synced byte offset，并维护 appended/synced replication offset 映射；显式 sync 会先 flush 再执行 `fsync(2)`，BGREWRITEAOF 切换时保留逻辑写入位置并重置 synced；`WAITAOF` 本地确认已接线，自动 `no/always/everysec` 策略继续后置
...
- [x] List 批量搬移：`LMOVEM source destination LEFT|RIGHT LEFT|RIGHT [COUNT|EXACTLY count OBO|BULK]` 与 `BLMOVEM ... timeout [...]`，覆盖默认单元素数组、数量上限、非阻塞 `EXACTLY` 不足时 Null Array/no-op、阻塞 `EXACTLY` 完整数量唤醒、目标顺序、同 key 批量旋转、双 key flags 与 AOF replay
...
- [x] Set 第五批基数读取：`SINTERCARD`、`SDIFFCARD`、`SUNIONCARD`，覆盖动态 `numkeys`、`LIMIT`、空 key、错类型、参数错误、`COMMAND GETKEYS*` 与 redis-py/redis-cli smoke；`SUNIONCARD APPROX` 当前使用精确集合回退
...
- [x] Blocking list / zset：已补 `BLPOP`、`BRPOP`、`BRPOPLPUSH`、`BLMOVE`、`BLMOVEM`、`BLMPOP`、`BZPOPMIN`、`BZPOPMAX`、`BZMPOP`
...
- [ ] Functions / Script 收口：只读脚本第一批已补 `EVAL_RO` / `EVALSHA_RO` single-call subset partial，并补齐 `SORT ... STORE` 等参数驱动写路径拒绝，`SCRIPT DEBUG` 已补 no-op 兼容面，`SCRIPT KILL` 已补无运行脚本 `NOTBUSY` 错误面 partial，脚本缓存、错误中止与实际数据命令的 AOF/复制传播边界已补；Functions 第一批已补 `FUNCTION HELP/LIST/STATS/FLUSH/DELETE/LOAD/DUMP/RESTORE/KILL`，并完成单 library/最多 8 个位置参数或 table 形式 `redis.register_function(... return redis.call(...))` 子集的进程内存储与 `FCALL/FCALL_RO` 执行，table 注册支持 bounded description、`no-writes` flag、`LIST` 元数据真值和运行时拒写；`FUNCTION DUMP/RESTORE` 已补 Redis 7 `FUNCTION2` 非空 payload、RDB length/version/CRC64、Redis LZF 输入、`APPEND/REPLACE/FLUSH` 和 staging 原子提交 partial。其他 flags、library 元数据 RDB/AOF/复制持久化和完整 Lua engine 仍待后续批次；Hash field TTL 运行期、持久化与复制传播语义已补 `HGETEX/HSETEX/HEXPIRE/HPEXPIRE/HEXPIREAT/HPEXPIREAT/HTTL/HPTTL/HEXPIRETIME/HPEXPIRETIME/HPERSIST`
- [ ] ACL 第一批：控制面、默认/named user、两类用户的 Redis 安全整体 `reset`、默认用户 `on/off`、`allcommands/nocommands`、基础 `-@all` whitelist 与有序 command/category allow/deny、key/channel pattern、多 SHA-256 口令、审计日志和 aclfile 原子 SAVE/全状态回滚 LOAD 已补；用户名已按字节区分大小写，只有精确小写 `default` 指向内建用户，其他大小写变体可作为独立 named user 完成认证、授权、文件 round-trip 和删除；两类用户的 `off` 均保留既有认证会话但阻止新连接或后续 AUTH，默认 enabled/nopass 同步进入 GETUSER/LIST/SAVE/LOAD 与失败回滚；`ACL DELUSER` 已支持一次删除多个用户时主动断开全部匹配连接，自删除先回复后关闭；`ACL LOAD` 成功后会断开新文件中已消失用户的全部会话，自移除先回复 `OK` 再关闭；默认用户 `reset` 恢复 `off/resetpass/resetkeys/resetchannels/-@all` 且清空 selector，默认和 named user 的整条 `ACL SETUSER` 已补全状态快照，modifier 语法、容量或口令删除失败不再留下部分变更；每用户最多 8 个 selector 已补，同一 selector 内完整匹配、多个 selector 与基础规则 OR 授权，普通命令、事务、脚本与 DRYRUN 共用统一判定；方向性 key pattern（`%R~/%W~/%RW~`）已按 operation 标签接入固定键和动态 source/destination，支持重复 glob 权限合并、规范化输出、selector 与 ACL 文件 round-trip；动态键已覆盖脚本/函数、MPOP、set cardinality、MSETEX、zset 聚合/store、`GEOSEARCHSTORE`、`GEORADIUS[BYMEMBER] STORE/STOREDIST`、`ZRANGESTORE`、`XREAD/XREADGROUP`、`XGROUP/XINFO` key 子命令、`SORT [STORE]` 与 `MEMORY USAGE`。后续随命令实现继续审计其余动态 key spec / movablekeys 和 operation 标签
- [ ] 运维与诊断第一批：`CONFIG` 余量（`timeout` 已补运行时回读、重写和普通空闲连接关闭，`latency-tracking` 运行时开关、`latency-monitor-threshold` 事件门限、`slowlog-log-slower-than` 慢日志门限和 `slowlog-max-len` 保留长度已补）、`CLIENT` 余量（`INFO/LIST` 已补真实 `addr/laddr/fd/age/idle/cmd/redir/events/io-thread/tot-net-in/multi/watch/sub/psub/ssub/qbuf/qbuf-free/argv-mem/multi-mem/rbs/rbp/obl/oll/omem/tot-mem` 观测、单数据库 `db=0`、ACL `user` 与当前支持连接状态 `N/O/P/x/b/t/B/d/e/T flags` 真值、`LIST ID id...` 多值过滤、`LIST TYPE NORMAL|PUBSUB` 实时订阅分类及 `MASTER|REPLICA|SLAVE` standalone 空结果兼容，legacy `KILL ip:port` 单目标及现代 `KILL ID/ADDR/LADDR/USER/TYPE/MAXAGE/SKIPME` 交集过滤，其中 LADDR 支持按服务端本地监听地址多目标关闭，USER 支持按 ACL 用户多目标关闭，TYPE 支持 NORMAL/PUBSUB 实时分类和 standalone 复制类型空结果，MAXAGE 支持有符号 64 位正整数秒、整秒 `age >= threshold` 边界和重复 selector 末值生效，并覆盖缺失地址、未知用户/类型、MAXAGE 精确错误与默认跳过当前连接，`REPLY` 命令回复抑制 partial、`UNBLOCK` 阻塞 pop 等待解除 partial、`PAUSE WRITE` 读写区分及非法 timeout 完整错误文本、`TRACKING` 连接级 flags/redirect/`BCAST PREFIX` 状态、WATCH key 计数与 dirty-CAS 冷路径真值、Redis 8.0 单事件循环 `io-thread=0`、socket 累计输入 `tot-net-in`、`CACHING` / `NO-EVICT` 连接级兼容标志与 `NO-TOUCH` 对象访问 LRU/LFU touch 抑制已补）、`MEMORY` 观测（`MALLOC-STATS` runtime-approx、`PURGE` no-op 兼容面与容器 arity 完整错误文本已补）、`SLOWLOG` 容器 arity 完整错误文本、`LATENCY`（含 top-level 与容器子命令直方图）、`MONITOR`、`MODULE HELP/LIST` 空模块兼容面及容器 arity 完整错误文本已补，`MODULE LOAD/LOADEX/UNLOAD` 与 Redis Array `AR*`、RediSearch `FT.*`、RedisJSON `JSON.*`、Redis Vector Set `V*`、RedisBloom `BF.*` / `CF.*` / `CMS.*` / `TOPK.*` / `TDIGEST.*`、RedisTimeSeries `TS.*` 已按单机安全 profile 收口为 `standalone-error`，`DEBUG` 已按单机安全 profile 收口为 `standalone-error`，`HOTKEYS HELP/GET/RESET/START/STOP` 已补空采样/no-op 诊断兼容面 partial，Sharded Pub/Sub 已补 `SSUBSCRIBE/SUNSUBSCRIBE/SPUBLISH` standalone 连接级 partial 与 `PUBSUB SHARDCHANNELS/SHARDNUMSUB` 真值，后续继续补配置、客户端与观测真值
- [ ] 复制/持久化深化：RDB 二进制兼容推进、AOF rewrite 压测、复制断线重连、`SYNC` legacy full-sync bulk snapshot partial 已补；`REPLCONF ACK [FACK]` 已补整数校验、无普通回复、仅已建立 `PSYNC` 的复制连接记录单调 ACK/FACK offset，`GETACK *` 已补 replica 按当前 upstream/synced replication offset 写回 ACK/FACK；`WAIT` 已补普通 ACK 等待，`WAITAOF` 已补真实本地 fsync、FACK 聚合、按需 GETACK、阻塞唤醒与 timeout；副本只读已补默认 `replica-read-only yes`、静态/动态写路径和事务 READONLY、CONFIG/INFO 热切换、提升 master 与内部复制回放边界。后续继续补周期性 GETACK、配置型 pull replica 的复制回放 AOF/FACK 闭环、`MIGRATE` 当前已按单机安全 profile 收口为 `standalone-error`，`FAILOVER` 当前已收口为无副本/未支持 controlled failover 错误面，后续再补真实 failover 状态机
