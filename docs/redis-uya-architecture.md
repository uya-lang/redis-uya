...
- `ACL CAT/DELUSER/GENPASS/HELP/LOAD/LOG/SAVE/USERS/WHOAMI` 仍由 `command/executor.uya` 执行；`ACL SETUSER/DRYRUN/LIST/GETUSER` 在真实连接路径由 `connection.uya` 接管默认用户的进程内命令 deny list，以便在命令执行前统一返回 `NOPERM`。当前支持默认用户命令级 `+cmd/-cmd` 允许/拒绝、`ACL LIST` / `ACL GETUSER default` 回显 deny list、事务队列前置拒绝和脚本内部命令拒绝；ACL 用户存储、密码管理、分类权限、key pattern 权限、真实 ACL 日志和 ACL 文件加载保存后续再补
